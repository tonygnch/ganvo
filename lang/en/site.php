<?php

return [
    'units' => [
        'sqm' => '/ m²',
        'lm' => '/ lm',
        'cbm' => '/ m³',
    ],
    'lang' => [
        'switch' => 'Language',
        'en' => 'English',
        'bg' => 'Български',
    ],

    'currency' => [
        'switch' => 'Currency',
    ],

    'common' => [
        'cookies' => [
            'label' => 'Cookies',
            'text' => 'We use optional Google Analytics cookies (kept for up to 13 months) to understand how the site is used. No ads, no cross-site tracking. You can change your choice anytime via the “Cookies” link in the footer.',
            'accept' => 'Accept',
            'decline' => 'Decline',
            'settings' => 'Cookies',
        ],
        'sign_in' => 'Sign in',
        'sign_out' => 'Sign out',
        'create_account' => 'Create account',
        'start_free' => 'Start free',
        'continue_shopping' => 'Continue shopping',
        'shop' => 'Shop',
        'cart' => 'Cart',
        'my_account' => 'My account',
        'hi_name' => 'Hi, :name',
        'powered_by' => 'Powered by :brand',
        'free' => 'Free',
        'usd' => 'USD',
        'all_rights' => 'All rights reserved.',
        'or' => 'or',
        'optional' => 'optional',
        'back' => 'Back',
    ],

    'marketing' => [
        'title' => 'Ganvo — Web design & development studio',
        'meta_description' => 'Ganvo creates fast, distinctive and dependable websites and web applications for businesses ready to grow. Let’s talk about your project.',
        'nav' => [
            'services' => 'Services',
            'work' => 'Work',
            'contact' => 'Contact',
            'book' => 'Let’s talk',
        ],
        'sections' => [
            'home' => 'Home',
            'services' => 'Services',
            'work' => 'Work',
            'studio' => 'Web Studio',
            'process' => 'Process',
            'contact' => 'Contact',
            'about' => 'About',
        ],
        'hero' => [
            'kicker' => 'Ganvo · Web design & development studio',
            'headline' => 'Web experiences,',
            'headline_accent' => 'built for growth.',
            'sub' => 'We create fast, distinctive and dependable websites and web applications built around your business goals.',
            'cta_primary' => 'Let’s talk',
            'cta_secondary' => 'Explore our work',
            'meta_primary' => 'EST 2024 · Web Studio',
            'cue' => 'Explore',
        ],
        'loader' => 'Loading',
        'about' => [
            'eyebrow' => 'Who are we?',
        ],
        'statement' => 'We create and improve web products that are clear, fast and built to perform — without compromise.',
        'services' => [
            'eyebrow' => 'Services',
            'heading' => 'Solutions with a clear purpose.',
            'items' => [
                ['title' => 'Websites, made to fit', 'body' => 'Websites designed for your brand and developed for speed, accessibility and sustainable growth.'],
                ['title' => 'Website redesign', 'body' => 'A strategic overhaul of your existing site with a clearer presence, modern architecture and measurably better performance.'],
                ['title' => 'Custom web applications', 'body' => 'Client portals, admin dashboards and internal tools shaped around the way your team works.'],
                ['title' => 'Support & evolution', 'body' => 'Ongoing technical care, monitoring and improvement to keep your site fast, secure and current.'],
                ['title' => 'SEO & performance', 'body' => 'Technical SEO foundations with clean code, fast load times and a clear structure that helps the right people find you.'],
            ],
        ],
        'work' => [
            'eyebrow' => 'Selected work',
            'heading' => 'Work that delivers.',
            'lead' => 'A focused selection of projects delivered in-house, from the first idea to a successful launch.',
            'visit' => 'Visit site',
            'pause' => 'Pause the project showcase',
            'open' => 'Explore our work',
            'count' => ':count projects',
            'close' => 'Close',
            'scenes' => [
                ['slug' => 'kass', 'name' => 'Kass Photography', 'type' => 'Photography · Portfolio', 'url' => 'https://kass.photography', 'tagline' => 'A dark, editorial portfolio that puts the photography first — galleries, campaigns and bookings, engineered to load instantly.'],
                ['slug' => 'midibg', 'name' => 'MidiBG', 'type' => 'Seafood · Brand site', 'url' => 'https://midibg.com', 'tagline' => 'A cinematic, scroll-driven brand site for a Black Sea seafood producer — story, motion and performance held in balance.'],
                ['slug' => 'asg', 'name' => 'Acoustic Sound Group', 'type' => 'Events · Portfolio', 'url' => 'https://acoustic-sound-group.com', 'tagline' => 'A precise site and booking experience for a sound and events company — clean architecture, built to scale with the business.'],
            ],
        ],
        'why' => [
            'eyebrow' => 'Why Ganvo',
            'heading' => 'We think strategically. We build precisely.',
            'items' => [
                ['title' => 'Built to fit', 'body' => 'No off-the-shelf templates or unnecessary limits. Every detail is made for your project and remains yours.'],
                ['title' => 'Speed by default', 'body' => 'Fast loading is part of a great experience. We optimise for strong performance from the foundation up.'],
                ['title' => 'Modern architecture', 'body' => 'Clean, component-driven code on a dependable stack that stays easy to maintain long after launch.'],
                ['title' => 'Ready to grow', 'body' => 'A foundation that can grow with your business — from a focused website today to a full platform tomorrow.'],
                ['title' => 'Care that continues', 'body' => 'We stay involved after launch with support, improvements and security you can rely on.'],
                // ['title' => 'Direct to the engineer', 'body' => 'You talk to the person building your site — not an account manager relaying messages.'],
            ],
        ],
        'process' => [
            'eyebrow' => 'Process',
            'heading' => 'A clear process. A confident result.',
            'steps' => [
                ['title' => 'Discovery', 'body' => 'We turn your goals, audience and constraints into a clear brief.'],
                ['title' => 'Planning', 'body' => 'We agree the scope, architecture and timeline before the build begins.'],
                ['title' => 'UX/UI Design', 'body' => 'We shape the interface and interactions around real content, then review them with you early.'],
                ['title' => 'Development', 'body' => 'We build every part with care, focusing on speed, quality and longevity.'],
                ['title' => 'Testing', 'body' => 'We test across devices, accessibility and performance — leaving nothing to assumption.'],
                ['title' => 'Launch', 'body' => 'A calm, monitored launch with every detail ready to go.'],
                ['title' => 'Support', 'body' => 'We continue with care, improvements and a direct line whenever you need us.'],
            ],
        ],
        'contact' => [
            'eyebrow' => 'Let’s talk',
            'heading' => 'Let’s turn your idea into a working product.',
            'sub' => 'Tell us about your project. You’ll receive a personal response from the specialist who would work on it — usually within one business day.',
            'assurances' => [
                'Reply within one business day',
                'A direct line to your specialist',
                'A clear quote, tailored to your project',
            ],
            'email' => 'hello@ganvo.bg',
            'phone' => '',
            'instagram' => '',
            'facebook' => '',
            'x' => 'https://x.com/Ganvox1',
            'direct' => 'Prefer email?',
            'thanks' => 'Thanks — we’ve received your message. We’ll be in touch within one business day.',
            'error_captcha' => 'We couldn\'t verify you\'re human. Please try again.',
            'error_throttled' => 'You’ve sent a few requests in a row. Please try again in a minute.',
            'error_generic' => 'Something went wrong. Please try again in a moment.',
            'form' => [
                'name' => 'Your name',
                'email' => 'Email',
                'company' => 'Company (optional)',
                'project_type' => 'What do you need?',
                'budget' => 'Budget',
                'message' => 'About your project',
                'submit' => 'Send enquiry',
                'sending' => 'Sending…',
                'choose' => 'Select…',
            ],
            'types' => [
                'website' => 'New website',
                'redesign' => 'Website redesign',
                'webapp' => 'Web application',
                'other' => 'Something else',
            ],
            'budgets' => [
                'under-5k' => 'Under €5,000',
                '5-15k' => '€5,000–15,000',
                '15-40k' => '€15,000–40,000',
                '40k-plus' => '€40,000+',
                'unsure' => 'Not sure yet',
            ],
        ],
        'footer' => [
            'tagline' => 'Ganvo is a web design and development studio creating fast, distinctive and dependable digital products.',
        ],
        'coming_soon' => [
            'eyebrow' => 'Launching soon',
            'title' => 'Beautiful storefronts, launching soon.',
            'headline_1' => 'Your website.',
            'headline_2' => 'Coming soon.',
            'lead' => 'A white-label e-commerce platform where you launch a stunning storefront in minutes — pick a theme, customize, start selling. We\'re putting the final touches on. Be first to know when we open.',
            'email_placeholder' => 'you@yourbrand.com',
            'notify' => 'Notify me',
            'thanks' => "Thanks — we'll be in touch.",
            'helper' => 'No spam. One email when we launch.',
            'meta_description' => 'Ganvo is a white-label e-commerce SaaS launching soon. Sign up to be notified.',
            'error_throttled' => 'Too many requests — please try again in a minute.',
            'error_generic' => 'Something went wrong. Please check your email and try again.',
            'error_network' => 'Couldn\'t reach the server. Please try again.',
        ],
    ],

    'storefront' => [
        'announce' => 'Free shipping on orders over :amount — and easy returns within :days days.',
        'mode_toggle' => 'Switch light/dark mode',
        'nav' => [
            'shop' => 'Shop',
            'featured' => 'Featured',
        ],
        'hero' => [
            'eyebrow' => 'New season · :year',
            'headline' => 'Built for :tenant fans.',
            'sub' => 'Hand-picked products, thoughtful design, and the prices you actually want to see. Browse the collection — your favorites are waiting.',
            'cta_primary' => 'Shop the collection →',
            'cta_secondary' => "See what's new",
        ],
        'value_props' => [
            'shipping_title' => 'Free shipping',
            'shipping_sub' => 'On orders over :amount',
            'returns_title' => 'Easy returns',
            'returns_sub' => '30-day no-questions policy',
            'checkout_title' => 'Fast checkout',
            'checkout_sub' => 'Secure, takes under a minute',
        ],
        'featured' => [
            'eyebrow' => 'Just for you',
            'h2' => 'Featured products',
            'browse_all' => 'Browse all →',
            'badge' => 'Featured',
        ],
        'shop_all' => [
            'eyebrow' => 'The whole collection',
            'h2' => 'Shop everything',
        ],
        // Forma theme — single-product hardware storefront.
        'forma' => [
            'related_eyebrow' => 'Complete the kit',
        ],
        // Kiln theme — studio/ceramics editorial sections (home only).
        'kiln' => [
            'hero_eyebrow' => 'Hand-thrown studio ceramics',
            'hero_lede' => 'A small collection of wheel-thrown stoneware — released in limited runs, each piece quietly different from the last.',
            'index_label' => 'Selected work',
            'meta_1_b' => 'Wheel-thrown',
            'meta_1_t' => 'in small batches',
            'meta_2_b' => 'Food-safe',
            'meta_2_t' => 'glazes throughout',
            'meta_3_b' => 'Limited',
            'meta_3_t' => 'seasonal runs',
            'meta_4_b' => 'Wrapped',
            'meta_4_t' => 'by hand',
            'recent_h2' => 'Recent work',
            'recent_more' => 'All pieces →',
            'maker_kicker' => 'From the studio',
            'maker_quote' => '“I make things to be used — chipped, stained, lived with. A pot that gathers a little history is doing exactly what it should.”',
            'maker_sign' => 'Hand-thrown · One of a kind',
            'process_kicker' => 'The process',
            'process_h3' => 'Thrown, trimmed, glazed,',
            'process_h3_em' => 'fired',
            'process_h3_tail' => '— twice.',
            'process_p' => 'Every piece passes through the same slow rhythm over a fortnight. Because each is shaped by hand, no two will ever be identical.',
            'step_1_b' => 'Thrown',
            'step_1_p' => 'On the wheel, in small daily batches.',
            'step_2_b' => 'Trimmed & dried',
            'step_2_p' => 'Footed, refined, left to firm up.',
            'step_3_b' => 'Glazed & fired',
            'step_3_p' => 'Bisque, hand-glazed, fired to 1260°C.',
            'process_cta' => 'Shop the work',
            'news_h3' => 'Be first to the next',
            'news_h3_em' => 'release',
            'news_p' => 'New runs are small and tend to sell out. We’ll let you know before they go live.',
            'related_eyebrow' => 'From the same shelf',
            'pill_wheel' => 'Wheel-thrown',
            'pill_stoneware' => 'Stoneware',
            'pill_unique' => 'One of a kind',
        ],
        // Sediment theme — natural-wine cellar accents.
        'wick' => [
            'related_eyebrow' => 'Burns well beside',
            'pill_low' => 'Small-batch soy',
            'pill_poured' => 'Poured by hand',
            'pill_hand' => 'Cotton wicks',
        ],
        // Timber theme — treated-wood / building-materials yard accents.
        'timber' => [
            'spec_note' => 'Pressure-treated to spec',
            'related_eyebrow' => 'From the same rack',
            'pill_treated' => 'Pressure-treated',
            'pill_graded' => 'Strength-graded',
            'pill_cut' => 'Cut to length',
            'uc_eyebrow' => 'Treatment guide',
            'uc_h2' => 'Built for where it goes',
            'uc1_title' => 'Indoors, dry',
            'uc1_desc' => 'Framing, furniture and interior joinery kept permanently dry.',
            'uc2_title' => 'Indoors, damp risk',
            'uc2_desc' => 'Roof structures and interiors with occasional condensation.',
            'uc3_title' => 'Outdoors, above ground',
            'uc3_desc' => 'Cladding, fences and decking exposed to the weather.',
            'uc4_title' => 'Ground & water contact',
            'uc4_desc' => 'Posts, sleepers and piles set into soil or fresh water.',
            'bulk_eyebrow' => 'Trade & sites',
            'bulk_h2' => 'Building big? Talk to the yard.',
            'bulk_body' => 'Project quantities, cutting to size and scheduled site delivery — tell us what the drawings say and we’ll price the full list.',
            'bulk_row1' => 'Volume pricing',
            'bulk_row2' => 'Cut to length',
            'bulk_row3' => 'Site delivery',
            'bulk_cta' => 'Request a quote',
        ],
        // Sankevi — the forest-atelier theme. Written for a Rhodope sawmill,
        // but every string is merchant-overridable through the manifest.
        'sankevi' => [
            'hero_kicker' => '',
            'hero_h1_html' => 'From raw timber to a <em>finished product</em>',
            'hero_sub' => 'An established manufacturer of solid wood products, with many years of experience.',
            // The three things the yard does FOR the customer, in the order
            // they get asked: how will you cut it, how dry, when is it here.
            'hero_note' => '',
            'hero_cta' => 'Our products',
            'hero_cta2' => 'Ask for a quote',
            'gx_home' => 'The yard',
            'gx_product' => 'The plate',
            'ledger_stock' => 'Lines in the yard',
            'ledger_1_k' => 'Dried down to',
            'ledger_1_v' => '12–15%',
            'ledger_2_k' => 'Structural grade',
            'ledger_2_v' => 'C24',
            'ledger_3_k' => 'Cut to size within',
            'ledger_3_v' => '48 h',
            'species_eyebrow' => 'Species & use',
            'species_h2_html' => 'Every board knows <em>where it is going</em>',
            'density_label' => 'Density',
            'sp1_name' => 'Spruce',
            'sp1_lat' => 'Picea abies',
            'sp1_use' => 'Framing, roof structures and formwork — light, straight and easy to work.',
            'sp1_dens' => '450 kg/m³',
            'sp2_name' => 'Scots pine',
            'sp2_lat' => 'Pinus sylvestris',
            'sp2_use' => 'Decking, fencing and cladding; it takes treatment deeper than anything else we cut.',
            'sp2_dens' => '520 kg/m³',
            'sp3_name' => 'Oak',
            'sp3_lat' => 'Quercus petraea',
            'sp3_use' => 'Floors, beams and garden furniture meant to outlive whoever ordered them.',
            'sp3_dens' => '720 kg/m³',
            'sp4_name' => 'Beech',
            'sp4_lat' => 'Fagus sylvatica',
            'sp4_use' => 'Worktops, stair treads and joinery — dense and even, and it planes to a glass finish.',
            'sp4_dens' => '700 kg/m³',
            'forest_quote' => 'We take only what the mountain can spare — and we cut it once.',
            'forest_caption' => 'Rhodope mountains · 1,400 m',
            'story_eyebrow' => 'The workshop',
            // Deliberately WITHOUT a generation count: this band now sits
            // directly above the merchant's own story and timeline on
            // /about, and platform copy claiming "four" over their own
            // "third generation" contradicts itself on one screen.
            'cert_alt' => 'FSC® chain-of-custody certification',
            'cert_line' => 'Certified timber',
            'cert_code' => 'FSC® CU-COC-879804',
            'story_h2_html' => 'One family, <em>the same saw</em>',
            'story_body' => 'The mill has stood in this clearing since our great-grandfather set the first blade in it. We still stack every board by hand, still dry it slowly, and still turn down the log that will not hold its shape.',
            'story_cta' => 'Our story',
            'trade_eyebrow' => 'Trade & sites',
            'trade_h2' => 'Building big? Bring us the drawings.',
            'trade_body' => 'Write or call us — every enquiry is answered personally.',
            'trade_row1' => 'Volume pricing',
            'trade_row2' => 'Cut to length',
            'trade_row3' => 'Delivery to site',
            'trade_cta' => 'Request a quote',
            'related_eyebrow' => 'From the same stack',
            'card_action' => 'View',
            'pdp_note' => '',
            'pill_kiln' => 'Kiln-dried',
            'pill_graded' => 'Strength-graded',
            'pill_cut' => 'Cut to size',

            // ——— THE LANDING PAGE AS A BRAND PAGE. Home is no longer a
            // shop window: the catalogue lives at /shop. The strings below
            // carry the acts of the cinematic landing — hero, velocity band,
            // flowing family menu, story, forest and the closing call.
            'scroll_cue' => '',
            'marquee_name' => '',
            'marquee_trade' => 'Manufacturer of high-quality solid wood products',
            'marquee_place' => '',
            'marquee_since' => 'since :year',
            // What a screen reader gets instead of the decorative running band.
            'marquee_sr' => ':name — manufacturer of high-quality solid wood products.',
            'families_eyebrow' => 'What we cut',
            'families_h2_html' => 'Solid <em>wood products</em>',
            // The hint now describes the gesture, not a row: the full-width
            // flowing menu became a compact option wheel.
            'families_hint' => 'Turn the wheel · ↑ ↓',
            // Same hint for the phone, where the wheel lies down.
            'families_all' => 'The whole yard',
            // The wheel's accessible name, and what is spoken politely when it
            // is turned by pointer or scroll with focus outside the control.
            'wheel_label' => 'Product families',
            'wheel_selected' => 'Selected: :name',
            // pickup_* are no longer referenced: the "off the stack" trio is
            // gone and the landing page shows no products at all. The keys
            // stay so a theme or translation still reading them cannot break.
            'pickup_eyebrow' => 'Off the stack this week',
            'pickup_all' => 'See everything in the yard',
            'closing_eyebrow' => 'Next step',
            'closing_h2_html' => 'Get <em>in touch</em>',
            'closing_shop' => 'Contact us',

            // ——— WHAT WE OFFER. The family story moved wholesale to /about;
            // the landing page now talks about what the yard can DO, and why
            // you would place the order here rather than anywhere else.
            'offer_eyebrow' => 'Capabilities',
            'offer_h2_html' => 'The <em>production process</em>',
            'offer_lead' => 'From raw timber to a finished product.',
            'offer_1_h' => 'Sawn to spec',
            'offer_1_p' => 'We cut to your schedule of sections, not to a standard catalogue. Send the sizes — we send back quantities and a price.',
            'offer_2_h' => 'Kiln drying',
            'offer_2_p' => 'Brought down to 10–12% moisture. Board that will not cup or twist once it is already fixed in place.',
            'offer_3_h' => 'UC3 / UC4 treatment',
            'offer_3_p' => 'Our own autoclave for anything that stays outdoors or touches the ground — fencing, decking, structural frames.',
            'offer_4_h' => 'Profiling',
            'offer_4_p' => 'Flooring, panelling, skirting and cladding to our own profiles. We will grind a non-standard knife from your sample.',
            'offer_5_h' => 'Strength grading',
            'offer_5_p' => 'Structural timber graded to C24, with the class stated on every delivery instead of “whatever came off the saw”.',
            'offer_6_h' => 'Delivery to site',
            'offer_6_p' => 'Our own trucks across southern Bulgaria, unloading on a schedule agreed with the site.',

            // ——— WHY US. Reasons, not a biography: the family history lives
            // at /about and is pointed at from here with a single link.
            // ——— THE FACTS IN THE "WHY US" BAND.
            // Every number is printed in the client's catalogue or is a direct
            // count of a list it enumerates. The value is free text: leading
            // digits animate, the remainder is shown as a suffix.
            'fact_1_value' => '25+',
            'fact_1_label' => 'Years in woodworking',
            'fact_2_value' => '20+',
            'fact_2_label' => 'Years on the Dutch market',
            'fact_3_value' => '30+',
            'fact_3_label' => 'Products in the catalogue',

            'why_eyebrow' => 'Why us',
            'why_h2_html' => 'Our <em>strengths</em>',
            'why_1_h' => 'Certified timber',
            'why_1_p' => 'FSC (Forest Stewardship Council) certification is an international standard proving that timber, paper and other forest products come from responsibly managed, environmentally sound and socially equitable forests. It guarantees traceability of the material from the forest to the end customer.',
            'why_2_h' => '25 years for the Dutch market',
            'why_2_p' => 'We have worked with the Dutch market for 25 years. Reliable production and export experience stand behind our long-term partnerships with customers across Europe.',
            'why_3_h' => 'The complete production process',
            'why_3_p' => 'Initial cutting, kiln drying and bringing the timber down to 12% moisture. Calibrating to exact size, packing and transport to the end customer.',
            'why_4_h' => '',
            'why_4_p' => '',
            'why_about' => '',

            // ——— THE STOCK BOOK (/shop). The catalogue is its own
            // destination, not a card grid: one row per item, the way the
            // mill's own book runs — photograph, sizes, availability, price.
            'shop_eyebrow' => 'The stock book',
            'shop_h1_html' => 'Everything off the <em>saw</em>',
            'shop_h1_search_html' => 'Search for :q',
            'shop_intro' => 'One row, one item in the yard: the sizes we mill it in, and the price of the smallest section. Open a row for the full specification.',
            'shop_refine' => 'Refine',
            'shop_all' => 'All products',
            'shop_choose' => 'Choose by :axes',
            'shop_from' => 'from',
            'shop_open' => 'View the entry',
            'shop_empty_h_html' => 'Nothing in <em>the book</em>',
            'shop_empty_p' => 'No row matches this selection. Drop a filter — or write to us: we cut to specification too.',
            'shop_empty_cta' => 'Clear the selection',

            // ——— ENQUIRY FLOW (Store::FLOW_ENQUIRY). This yard takes no money
            // online: the customer sends a cutting list and is called back with
            // a quote. Nothing below may imply a charge, a card or a payment —
            // that is the whole point of the wording. Used by cart / checkout /
            // order when $store->isEnquiryFlow() (resp. $order->order_flow).
            'req_gx' => 'The list',
            'req_crumb' => 'Request · nothing is charged online',
            'req_h1_html' => 'Your <em>cutting list</em>',
            'req_lead_one' => ':count line ready to go to the yard.',
            'req_lead_many' => ':count lines ready to go to the yard.',
            'req_empty_h_html' => 'Nothing on <em>the list</em> yet',
            'req_empty_p' => 'Pick your boards, set the lengths, and send us the list. We price it and come back to you — no payment happens on this site.',
            'req_empty_cta' => 'Into the yard →',
            'req_col_item' => 'Item',
            'req_col_qty' => 'Quantity',
            'req_col_est' => 'Estimate',
            'req_back' => 'Add more to the list',
            'req_panel_h' => 'Your request',
            'req_delivery_note' => 'Quoted with the offer',
            'req_est_label' => 'Estimate',
            'req_est_note' => 'An indicative figure taken from our list prices. The yard confirms the final price once the cut and the delivery are agreed.',
            'req_add' => 'Add to the request',
            'req_cta' => 'Continue to request →',
            'req_no_payment' => 'No card · no payment online',

            // Checkout — the wizard's third sheet is a review, not a till.
            'co_crumb' => 'Request · no payment is taken',
            'co_h1' => 'Send the list',
            'step_review' => 'Send',
            'sec_review' => 'Review & send',
            'rv_contact' => 'Contact',
            'rv_delivery' => 'Delivery',
            'rv_list' => 'The list',
            'rv_edit' => 'Edit',
            'rv_notes' => 'Notes',
            'rv_none' => '—',
            'next_h' => 'What happens next',
            'next_1' => 'We read the list and check the yard for every line on it.',
            'next_2' => 'We price it — timber, cutting and delivery — and put the offer in writing.',
            'next_3' => 'We call or email you, usually within one working day. Nothing is charged until you accept.',
            'no_pay_h' => 'Nothing is charged now',
            'no_pay_p' => 'This is a request, not an order. No card details are collected and no payment is taken on this site.',
            'send_request' => 'Send the request',

            // Order confirmation — a receipt for a request, never for a payment.
            'ord_title' => 'Your list is with us, :name',
            'ord_title_anon' => 'Your list is with us',
            'ord_body' => 'We have the request and we are pricing it now. The offer goes to :email — or we call, if that is quicker.',
            'ord_ref' => 'Request no.',
            'ord_state' => 'Request',
            'ord_state_pending' => 'Received · awaiting quote',
            'ord_state_confirmed' => 'Quoted · confirmed',
            'ord_submitted' => 'Sent',
            'ord_phone' => 'Phone',
            'ord_list_h' => 'The list you sent',
            'ord_est_total' => 'Estimated total',
            'ord_est_note' => 'Indicative only — our list prices, before the cut and the delivery are agreed. The offer we send is the binding figure.',
            'ord_back' => 'Back to the yard',

            /*
            |------------------------------------------------------------------
            | Rack configurator (/configurator)
            |------------------------------------------------------------------
            | Part names carry into the cart and freeze onto the order, so they
            | are written the way the yard says them, not the way a catalogue
            | would.
            */
            'cfg_nav' => 'Configurator',
            'cfg_title' => 'Shelving configurator',
            'cfg_eyebrow' => 'Build it yourself',
            'cfg_h1_html' => 'The rack, <em>built to your run</em>',
            'cfg_lead' => 'Pick a height, a depth and a number of levels, then add sections. The price moves as you build, and the parts list is exactly what we load onto the lorry.',

            'cfg_part_frame' => 'Frame :h×:d cm',
            'cfg_part_shelf' => 'Shelf :w×:d cm',
            'cfg_part_end_pin' => 'End pin Ø10',
            'cfg_part_extension_pin' => 'Extension pin Ø10',
            'cfg_part_cross_brace' => 'Cross-brace',
            'cfg_part_wine_tray' => 'Wine tray :w×:d cm',
            'cfg_part_desk_top' => 'Desk top :w×:d cm',
            'cfg_sections_count' => '{1} :count section|[2,*] :count sections',
            'cfg_link_stale' => 'This saved configuration is no longer available — here is a standard rack to start from.',
            'cfg_levels_count' => '{1} :count level|[2,*] :count levels',
            'cfg_rack_name' => ':model :h×:d cm · :sections × :levels · :metres m',

            // The rack models.
            'cfg_model' => 'Model',
            'cfg_model_single' => 'Rack',
            'cfg_model_office' => 'Office rack',
            'cfg_model_wine' => 'Wine rack',

            'cfg_step_height' => 'Frame height',
            'cfg_step_depth' => 'Depth',
            'cfg_step_levels' => 'Levels',
            'cfg_step_desk_depth' => 'Desk depth',
            'cfg_add_section' => 'Add section',
            'cfg_reset' => 'Start again',
            'cfg_reset_confirm' => 'Start a new configuration?',

            'cfg_section_n' => 'Section :n',
            'cfg_section_size' => ':w × :d cm',
            'cfg_edit_width' => 'Width',
            'cfg_move_left' => 'Left',
            'cfg_move_right' => 'Right',
            'cfg_delete_section' => 'Remove this section',
            'cfg_shared_note' => 'Height, depth and levels apply to the whole run.',

            'cfg_zoom_in' => 'Zoom in',
            'cfg_zoom_out' => 'Zoom out',
            'cfg_fit' => 'Centre',
            'cfg_view_label' => 'Rack elevation',

            'cfg_panel_h' => 'Your configuration',
            'cfg_summary_height' => 'Height',
            'cfg_summary_depth' => 'Depth',
            'cfg_summary_length' => 'Total length',
            'cfg_summary_sections' => 'Sections',
            'cfg_summary_levels' => 'Levels',
            'cfg_summary_shelves' => 'Shelves',
            'cfg_set_includes' => 'The set includes',
            'cfg_see_set' => 'See what the set includes',
            'cfg_qty' => 'pcs',

            'cfg_subtotal' => 'Price excl. VAT',
            'cfg_vat' => 'VAT :rate%',
            'cfg_total' => 'Total',
            'cfg_price_note' => 'An estimate at list prices. Transport is quoted separately.',

            'cfg_add_to_cart' => 'Add to the request',
            'cfg_added' => 'The rack has been added to your request',
            'cfg_save' => 'Save this configuration',
            'cfg_saved' => 'Configuration saved',
            'cfg_copy_link' => 'Copy link',
            'cfg_link_copied' => 'Link copied',
            'cfg_code_label' => 'Configuration code',
            'cfg_restored' => 'Restored configuration :code',

            'cfg_over_limit' => 'The largest configuration we can price online is :metres m. For a bigger project, get in touch for an individual quote.',
            'cfg_err_height' => 'That height is not available.',
            'cfg_err_depth' => 'That depth is not available.',
            'cfg_err_levels' => 'That number of levels is not available.',
            'cfg_err_width' => 'That section width is not available.',
            'cfg_err_no_sections' => 'A rack needs at least one section.',
            'cfg_err_too_long' => 'The largest configuration we can price online is :metres m. For a bigger project, get in touch for an individual quote.',
            'cfg_err_no_frame_price' => 'There is no frame for that height and depth at the moment.',
            'cfg_err_no_shelf_price' => 'There is no shelf for that width and depth at the moment.',
            'cfg_err_no_model_price' => 'This model has no price at that width and depth at the moment.',
            'cfg_err_type' => 'That model is not available.',
            'cfg_err_desk_depth' => 'The desk must be at least as deep as the rack.',
            'cfg_err_brace' => 'That cross-brace is for a section that does not exist.',
            'cfg_brace_auto' => 'This section always has a cross-brace.',
            'cfg_brace_add' => 'Add a cross-brace to this section',
            'cfg_desk' => 'Desk',
            'cfg_desk_add' => 'Put the desk on this section',
            'cfg_desk_last' => 'An office rack has at least one desk.',
            'cfg_err_desk_sections' => 'Choose at least one section for the desk.',
            'cfg_err_no_part_price' => 'A part of the set has no price. Get in touch and we will sort it out.',
            'cfg_err_not_found' => 'That configuration could not be found.',
            'cfg_err_generic' => 'Something went wrong. Try again.',

            // The quick sign-up window — keeping a rack takes an account.
            'cfg_auth_required' => 'Sign in to save or request a rack.',
            'cfg_auth_title' => 'Keep this rack in an account',
            'cfg_auth_lead' => 'You can find it again from any device, and we know who to talk to about the order.',
            'cfg_auth_tab_register' => 'New account',
            'cfg_auth_tab_login' => 'I have an account',
            'cfg_auth_register_btn' => 'Create account and continue',
            'cfg_auth_login_btn' => 'Sign in and continue',
            'cfg_auth_show_password' => 'Show password',
            'cfg_auth_close' => 'Close',

            // The 3D view of the same rack.
            'cfg_view_3d' => '3D',
            'cfg_view_2d' => '2D',
            'cfg_3d_label' => '3D model of the rack',
            'cfg_fullscreen' => 'Full screen',
            'cfg_fullscreen_exit' => 'Exit full screen',
        ],
        // Storefront contact page — merchant details + enquiry form.
        'contact' => [
            'title' => 'Contact',
            'heading' => 'Get in touch',
            'intro' => 'Questions about a product, an order or a delivery? Send us a message and we will come back to you.',
            'details_h' => 'Our details',
            'address_label' => 'Address',
            'phone_label' => 'Phone',
            'email_label' => 'Email',
            'hours_label' => 'Opening hours',
            'map_label' => 'Map',
            'form_h' => 'Send a message',
            'field_name' => 'Your name',
            'field_email' => 'Email',
            'field_phone' => 'Phone',
            'field_subject' => 'What is it about?',
            'field_message' => 'Message',
            'send' => 'Send message',
            'sending' => 'Sending…',
            'sent_title' => 'Message sent',
            'sent_body' => 'Thanks — we have your message and will reply to the email you gave us.',
            'error_throttled' => 'You have sent several messages already. Please try again a little later.',
            'subject_general' => 'General enquiry',
            'subject_quote' => 'Request a quote',
            'subject_order' => 'About an order',
            'subject_delivery' => 'Delivery',
            'subject_returns' => 'Returns',
        ],
        // Storefront history page — the merchant's own story, opt-in.
        'about' => [
            'title' => 'About us',
            'heading' => 'Our story',
            'intro' => 'A short history of who we are and how we got here.',
            'since' => 'Since :year',
            'milestones_h' => 'Along the way',
            'stats_h' => 'In numbers',
            'gallery_h' => 'A look inside',
            'image_alt' => ':name — photo :n',
            'cta' => 'Talk to us',
        ],
        'lookbook' => [
            'title' => 'The Lookbook',
            'hint' => 'Scroll to explore →',
        ],
        'promo' => [
            'h2_prefix' => 'Join the :tenant list',
            'p' => 'Get first dibs on new drops and exclusive subscriber-only offers.',
            'btn' => 'Sign up below ↓',
        ],
        'product' => [
            'breadcrumb_shop' => 'Shop',
            'quick_view' => 'Quick view',
            'wear_it_with' => 'Wear it with',
            'tax_included' => 'tax included',
            'in_stock_full' => 'In stock and ready to ship',
            'in_stock_low' => 'Only :count left in stock',
            'how_much' => 'How much do you need (:unit)',
            'covers' => '≈ :qty pcs — covers :measure',
            'price_range' => ':min – :max',
            'not_orderable_title' => 'Not available to order online',
            'not_orderable_body' => 'This item is sold by arrangement — get in touch about stock, sizes and price.',
            'not_orderable_cta' => 'Get in touch',
            'not_orderable_flash' => 'This item cannot be ordered online.',
            'quantity' => 'Quantity',
            'add_to_cart' => 'Add to cart',
            'wishlist' => 'Save for later',
            'perks' => [
                'shipping' => 'Free shipping on orders over :amount',
                'returns' => '30-day easy returns',
                'fast' => 'Ships within 1–2 business days',
            ],
            'in_stock_badge' => 'In stock',
            'no_image' => 'No image',
            // Image gallery / fullscreen viewer
            'view_fullscreen' => 'View full screen',
            'view_image' => 'View image :n of :total',
            'gallery_label' => 'Product image viewer',
            'close' => 'Close',
            'prev_image' => 'Previous image',
            'next_image' => 'Next image',
            // Variants
            'choose_variant' => 'Choose an option',
            'out_of_stock' => 'Out of stock',
            // Multi-axis picker: one group per option, values the current
            // selection rules out stay visible but disabled.
            'choose_option' => 'Choose :name',
            'option_unavailable' => 'Not available with your selection',
        ],
        'no_products' => 'No products yet. Check back soon.',
        // Toolbar above the product grid: search + sort + category + price.
        'controls' => [
            'search' => 'Search',
            'search_placeholder' => 'Search products…',
            'sort' => 'Sort by',
            'sort_category' => 'By category',
            'sort_newest' => 'Newest',
            'sort_price_asc' => 'Price · low to high',
            'sort_price_desc' => 'Price · high to low',
            'sort_name_asc' => 'Name · A → Z',
            'category' => 'Category',
            'category_more' => 'More about this category',
            'category_all' => 'All categories',
            'price' => 'Price',
            'price_min' => 'Min price',
            'price_max' => 'Max price',
            'availability' => 'Availability',
            'in_stock_only' => 'In stock only',
            'apply' => 'Apply',
            'clear' => 'Clear all',
            'no_results' => 'No products match your filters.',
            'result_count' => '{0} No products|{1} :count product|[2,*] :count products',
        ],
        'pagination' => [
            'aria_label' => 'Product pages',
            'prev' => 'Prev',
            'next' => 'Next',
        ],
        // Curated product groupings — strips on the home, dedicated pages.
        'collections' => [
            'heading' => 'Collections',
            'view_all' => 'View all',
            'empty' => 'No products in this collection yet.',
        ],
        'footer' => [
            'tagline' => 'Thoughtfully made. Honestly priced. Shipped fast.',
            'col_shop' => 'Shop',
            'all_products' => 'All products',
            'col_help' => 'Help',
            'shipping' => 'Shipping',
            'returns' => 'Returns',
            'contact' => 'Contact',
            'about' => 'About us',
            'newsletter_placeholder' => 'your@email.com',
            'subscribe' => 'Subscribe',
            'subscribed' => 'Subscribed ✓',
        ],
        'added_to_cart_measure' => 'Added: :name — :measure :unit',
        'added_to_cart' => 'Added :name to your cart.',
        'sign_in_for_checkout' => 'Please sign in to complete checkout.',
        // Variant-related cart messages
        'cart' => [
            'pick_a_variant' => 'Please choose an option before adding to your cart.',
        ],
        // Ember theme — specialty-coffee-roaster copy
        'ember' => [
            'related_eyebrow' => 'Pairs well with',
            'roast_label' => 'Roast level',
            'pill_origin' => 'Single-origin',
            'pill_whole_bean' => 'Whole bean',
            'pill_small_batch' => 'Small-batch',
        ],
    ],

    'cart' => [
        'title' => 'Your cart',
        'heading_html' => 'Your <em>Cart</em>',
        'your_selection' => 'Your selection',
        'item_count_one' => ':count item ready to check out.',
        'item_count_many' => ':count items ready to check out.',
        'empty_title' => 'Your cart is empty',
        'empty_sub' => "Looks like you haven't added anything yet.",
        'start_shopping' => 'Start shopping →',
        'continue_shopping' => 'Continue shopping',
        'unit_each_of' => ':price / :unit',
        'unit_piece' => 'pc',
        'unit_each' => ':price each',
        'decrease' => 'Decrease quantity',
        'increase' => 'Increase quantity',
        'quantity_label' => 'Quantity',
        'remove' => 'Remove',
        'shipping_at_checkout' => 'Calculated at checkout',
        'summary' => 'Order summary',
        'subtotal' => 'Subtotal',
        'shipping' => 'Shipping',
        'free_shipping_at' => 'Free shipping at :amount',
        'total' => 'Total',
        'tax_at_checkout' => 'Tax calculated at checkout',
        'checkout' => 'Checkout',
        'keep_shopping' => '← Keep shopping',
        'perk_shipping' => '⌁ Free shipping over :amount',
        'perk_returns' => '⟲ 30-day returns',
        'perk_fast' => '⚡ Ships in 1–2 days',
        // Discount-code input + flashes
        'discount_placeholder' => 'Discount code',
        'discount_apply' => 'Apply',
        'discount_remove' => 'Remove',
        'discount_applied' => ':name applied to your cart.',
        'discount_invalid' => "That code isn't valid for this cart.",
        'discount_removed' => 'Discount removed.',
    ],

    'checkout' => [
        'title' => 'Checkout',
        'back_to_cart' => '← Back to cart',
        'signed_in_as' => 'Signed in as :email',
        'have_account' => 'Have an account?',
        'sign_in_link' => 'Sign in',
        'for_faster' => 'for faster checkout.',
        'create_account_link' => 'Create account →',
        'sec_contact' => 'Contact',
        'sec_shipping' => 'Shipping address',
        'sec_shipping_method' => 'Shipping method',
        'sec_extras' => 'Order notes',
        'sec_payment' => 'Payment',
        'email' => 'Email',
        'full_name' => 'Full name',
        'phone' => 'Phone',
        'street' => 'Street address',
        'region' => 'State or region',
        'city' => 'City',
        'postal' => 'Postal code',
        'country' => 'Country',
        'notes_label' => 'Order notes',
        'notes_placeholder' => 'Anything we should know? Delivery hints, gift message…',
        'marketing_opt_in' => 'Send me product news and offers. I can unsubscribe anytime.',
        'stub_title' => 'Demo payment',
        'stub_body' => 'No real card details are collected. Clicking :action records this order as paid.',
        'action_place_order' => 'Place order',
        'secure_note' => 'Secure checkout · 30-day returns',
        'step_details' => 'Details',
        'step_delivery' => 'Delivery',
        'step_payment' => 'Payment',
        'wizard_continue' => 'Continue',
        'wizard_back' => 'Back',
        'summary' => 'Order summary',
        'pay_now' => 'Place order',
        'secure' => '🔒 Stub payment · no real charge',
        'secure_stripe' => '🔒 Secure payment by Stripe',
        'charged_in' => "You'll be charged :amount.",
    ],

    'order' => [
        'thanks_name' => 'Thank you, :name!',
        'thanks_anon' => 'Thank you for your order!',
        'shipped_title' => 'Your order is on the way',
        'cancelled_title' => 'Order cancelled',
        'refunded_title' => 'Order refunded',
        'cancelled_body' => "This order has been cancelled and won't be shipped.",
        'refunded_body' => 'A refund has been issued for this order.',
        'failed_title' => 'Payment failed',
        'failed_body' => "Your card was declined or the payment didn't complete. Head back to your cart to try again.",
        'pending_title' => 'Processing your payment…',
        'pending_body' => 'Your payment is being confirmed by your bank. This page will refresh automatically.',
        'shipped_body' => "Track your shipment below — it's on its way.",
        'paid_body' => "Your order is confirmed. We've sent a receipt to :email.",
        'shipped_via' => 'Shipped via :carrier',
        'tracking_meta' => 'Tracking :code · Sent :ago',
        'track_shipment' => 'Track shipment ↗',
        'details_h3' => 'Order details',
        'status' => 'Status',
        'placed' => 'Placed',
        'customer' => 'Customer',
        'email_label' => 'Email',
        'shipping_to' => 'Shipping to',
        'items_ordered' => 'Items ordered',
        'qty_unit' => 'Qty :qty · :price each',
        'subtotal' => 'Subtotal',
        'shipping_label' => 'Shipping',
        'total' => 'Total',
        'discount' => 'Discount',
        'you_saw' => 'You saw',
        'need_help' => 'Need help?',
    ],

    'auth' => [
        'login_title' => 'Welcome back',
        'login_lead' => 'Sign in to your :tenant account.',
        'register_title' => 'Create your account',
        'register_lead' => 'Sign up to track orders and check out faster at :tenant.',
        'email' => 'Email',
        'password' => 'Password',
        'password_hint' => 'Minimum 8 characters.',
        'password_confirm' => 'Confirm password',
        'full_name' => 'Full name',
        'sign_in_btn' => 'Sign in',
        'create_account_btn' => 'Create account',
        'new_here' => 'New here?',
        'create_account_link' => 'Create an account',
        'have_account' => 'Already have an account?',
        'sign_in_link' => 'Sign in',
        'bad_credentials' => 'These credentials do not match our records.',
        // Optional signup fields — only render when the store enables them.
        'optional' => 'optional',
        'phone' => 'Phone number',
        'birthday' => 'Birthday',
        'shipping_address' => 'Shipping address',
        'address_line' => 'Street address',
        'address_city' => 'City',
        'address_postal' => 'Postal code',
        'address_country' => 'Country (ISO code)',
        'marketing_consent' => 'Send me product news and offers from :tenant. I can unsubscribe anytime.',
        'email_taken' => 'There is already an account with this email. Sign in with your password.',
    ],

    'account' => [
        'racks' => 'My configurations',
        'racks_empty' => "You haven't saved a rack yet.",
        'racks_start' => 'Open the configurator →',
        'racks_saved_on' => 'Saved',
        'racks_open' => 'Open →',
        'hi' => 'Hi, :name',
        'signed_in_as' => 'Signed in as :email',
        'sign_out' => 'Sign out',
        'recent_orders' => 'Recent orders',
        'empty' => "You haven't placed any orders yet.",
        'start_shopping' => 'Start shopping →',
        'view' => 'View →',
        // Settings
        'settings' => 'Settings',
        'settings_title' => 'Account settings',
        'settings_lead' => 'Manage your details, address, and password.',
        'back_to_account' => 'Back to account',
        'profile_section' => 'Profile',
        'address_section' => 'Default shipping address',
        'password_section' => 'Password',
        'name' => 'Full name',
        'email' => 'Email',
        'phone' => 'Phone',
        'birthday' => 'Birthday',
        'address_line' => 'Street address',
        'address_region' => 'Region / state',
        'city' => 'City',
        'postal_code' => 'Postal code',
        'country' => 'Country',
        'marketing_optin' => 'Email me news and offers',
        'save_profile' => 'Save changes',
        'current_password' => 'Current password',
        'new_password' => 'New password',
        'confirm_password' => 'Confirm new password',
        'change_password' => 'Update password',
        'settings_saved' => 'Your details have been saved.',
        'password_changed' => 'Your password has been updated.',
        'wrong_password' => 'That current password is incorrect.',
        'optional' => 'optional',
    ],

    'email' => [
        'message_subject' => 'New message via :tenant — :subject',
        'message_greeting' => 'Hello,',
        'message_intro' => 'Someone used the contact form on :tenant.',
        'message_from' => '**From:** :name (:email)',
        'message_phone' => '**Phone:** :phone',
        'message_topic' => '**Subject:** :subject',
        'message_action' => 'Open messages',
        'message_outro' => 'Reply to this email to answer them directly.',
        'placed_subject' => 'Order :number confirmed — :tenant',
        'placed_greeting' => 'Hi :name,',
        'placed_thanks' => 'Thanks for your order from :tenant!',
        'placed_line_order' => '**Order number:** :number',
        'placed_line_total' => '**Total:** :amount :currency',
        'placed_line_item' => '• :name × :qty — :amount',
        'placed_line_discount' => 'Discount applied: :name:code — −:amount',
        'placed_action' => 'View order',
        'placed_outro' => "We'll email you again when it ships.",

        'shipped_subject' => 'Your order :number has shipped — :tenant',
        'shipped_greeting' => 'Hi :name,',
        'shipped_body' => 'Your order from :tenant is on its way!',
        'shipped_carrier' => '**Carrier:** :carrier',
        'shipped_tracking' => '**Tracking number:** :number',
        'shipped_track_action' => 'Track shipment',
        'shipped_view_action' => 'View order',

        'refunded_subject' => 'Refund issued for order :number — :tenant',
        'refunded_greeting' => 'Hi :name,',
        'refunded_full' => 'We\'ve issued a full refund of **:amount** for your order from :tenant.',
        'refunded_partial' => 'We\'ve issued a partial refund of **:amount** for your order from :tenant.',
        'refunded_eta' => 'Depending on your bank, the funds may take up to 5–10 business days to appear back in your account.',
        'refunded_view_action' => 'View order',
    ],

    'statuses' => [
        'pending' => 'Pending',
        'paid' => 'Paid',
        'shipped' => 'Shipped',
        'refunded' => 'Refunded',
        'cancelled' => 'Cancelled',
    ],

    'onboarding' => [
        'title' => 'Set up your store',
        'signup' => [
            'eyebrow' => 'Get started',
            'title' => 'Create your store',
            'lead' => 'Tell us about your store — you can change everything later.',
            'business_name' => 'Store name',
            'business_name_ph' => 'Acme Co.',
            'business_name_help' => "We'll use this as your storefront name and slug.",
            'your_name' => 'Your name',
            'email' => 'Work email',
            'password' => 'Password',
            'password_confirm' => 'Confirm password',
            'back' => 'Back to home',
            'cta' => 'Create my store',
            'have_account' => 'Already have an account?',
        ],
        'login' => [
            'eyebrow' => 'Welcome back',
            'title' => 'Sign in to your store',
            'lead' => 'Continue where you left off.',
            'email' => 'Email',
            'password' => 'Password',
            'back' => 'Back to home',
            'cta' => 'Sign in',
            'no_account' => "Don't have an account yet?",
            'invalid' => 'Those credentials did not match.',
        ],
        'steps' => [
            'business' => 'Business info',
            'plan' => 'Plan',
            'theme' => 'Theme',
            'customize' => 'Customize',
            'products' => 'First product',
            'launch' => 'Go live',
        ],
        'business' => [
            'eyebrow' => 'Step 1 of 6',
            'title' => 'Tell us about your store',
            'lead' => "We'll use this on your storefront and in support emails.",
            'store_name' => 'Store name',
            'store_name_help' => 'Your storefront URL will be :slug.ganvo.bg — change the name and slug stays the same.',
            'business_type' => 'What do you sell?',
            'business_type_ph' => 'Pick a category',
            'contact_email' => 'Contact email',
            'contact_phone' => 'Contact phone (optional)',
            'contact_phone_help' => "We won't share it — it just shows on order receipts.",
            'currency' => 'Base currency',
            'currency_help' => "What you'll price products in and get paid in. Customers can switch their view to another currency later.",
            'skip_home' => 'Save & continue later',
            'cta' => 'Continue',
        ],
        'plan' => [
            'eyebrow' => 'Step 2 of 6',
            'title' => 'Pick a plan',
            'lead' => 'Start free, upgrade whenever. Pricing is illustrative until billing is wired up.',
            'popular' => 'Most popular',
            'free' => 'Free',
            'per_month' => ' / mo',
            'per_year' => ' / yr',
            'billing_monthly' => 'Monthly',
            'billing_yearly' => 'Yearly',
            'you_save' => 'Save :amount vs paying monthly',
            'promo_ends' => 'Promo ends :date',
            'billed_annually_as' => 'Billed annually · :total',
            'back' => 'Back',
            'cta' => 'Continue',
            'skip' => 'Skip for now',
            'pay_now' => 'Pay now',
            'names' => [
                'starter' => 'Starter',
                'pro' => 'Pro',
                'business' => 'Business',
            ],
        ],
        'theme' => [
            'eyebrow' => 'Step 3 of 6',
            'title' => 'Choose a theme',
            'lead' => 'Live previews of your store. You can customize colors and fonts in the next step.',
            'selected' => 'Selected',
            'back' => 'Back',
            'cta' => 'Continue',
        ],
        'customize' => [
            'eyebrow' => 'Step 4 of 6',
            'title' => 'Make it yours',
            'lead' => 'Tweak the look on the left — the preview on the right updates as you go.',
            'primary' => 'Primary color',
            'primary_help' => 'Used for buttons, links, and accents.',
            'secondary' => 'Secondary color',
            'secondary_help' => 'Used for header backgrounds and dark accents.',
            'font' => 'Font family',
            'logo' => 'Logo (optional)',
            'logo_current' => 'Current logo. Upload a new file to replace it.',
            'logo_pending' => 'Selected — will show in the preview shortly.',
            'logo_help' => 'PNG or SVG, up to 2 MB. Looks best around 200×60.',
            'preview' => 'Live preview',
            'back' => 'Back',
            'cta' => 'Continue',
        ],
        'products' => [
            'eyebrow' => 'Step 5 of 6',
            'title' => 'Add your first product',
            'lead' => 'You can add more later — or skip and add them in the admin.',
            'name' => 'Product name',
            'name_ph' => 'Field Notebook',
            'price' => 'Price',
            'price_help' => 'In :currency. You can edit later.',
            'image' => 'Image (optional)',
            'description' => 'Description (optional)',
            'description_ph' => 'A linen-bound notebook for everyday thinking.',
            'cta_another' => 'Save & add another',
            'cta_continue' => 'Save & continue',
            'cta_skip' => 'Skip — I\'ll add products later',
            'cta_skip_more' => 'Continue without adding more',
            'added' => 'Product added. Add another or continue.',
            'added_count' => '{0} No products yet|{1} 1 product added|[2,*] :count products added',
            'none_yet' => 'Products you add will show up here.',
            'no_image' => 'No image',
        ],
        'launch' => [
            'eyebrow' => 'Step 6 of 6',
            'title' => 'Ready to launch',
            'lead' => 'Take one last look. Once you click launch, your storefront goes live at the URL below.',
            'edit' => 'Edit',
            'no_products' => 'No products yet (you can add them in the admin)',
            'product_count' => '{1} 1 product|[2,*] :count products',
            'url_intro' => 'Your storefront will live at:',
            'custom_domain_toggle' => 'Use my own domain instead (advanced)',
            'custom_domain_help' => "We'll set things up to use your domain. You'll add a DNS record from Store Settings after launch to finish verification.",
            'custom_domain_after' => 'Lowercase, no scheme. You can leave this blank and add a domain later.',
            'back' => 'Back',
            'cta' => 'Launch my store',
        ],
        'launched' => [
            'hooray' => "You're live",
            'title' => ':name is open for business',
            'lead' => 'Share the link, invite your first customers, and refine from the admin whenever.',
            'url_label' => 'Storefront',
            'visit' => 'Visit storefront',
            'customize' => 'Make it yours',
            'admin' => 'Open admin',
        ],
    ],
];
