...
@section('content')
    <style>
        ...
        .ord-hero .mark {
            ...
            border-radius: 2px; background: var(--ink); color: var(--bg);
            font-size: 32px; line-height: 1; position: relative;
        }
        /* gallery double-frame around the status mark */
        .ord-hero .mark::after { content: ""; position: absolute; inset: -8px; border: 1px solid var(--line2); border-radius: 2px; pointer-events: none; }
        .ord-hero .mark.danger { background: #9c5a3e; }
        .ord-hero h1 { font-family: var(--serif); font-weight: 400; font-size: clamp(34px, 5vw, 56px); line-height: 1.04; margin-bottom: 14px; letter-spacing: -.01em; }
        ...
        .ord-ship .ic { width: 48px; height: 48px; border-radius: 2px; background: var(--ink); color: var(--bg); display: grid; place-items: center; font-size: 22px; flex-shrink: 0; }
        .ord-ship .tx { flex: 1; min-width: 200px; }
        .ord-ship h3 { font-family: var(--serif); font-weight: 400; font-size: 18px; margin-bottom: 4px; color: var(--ink); }
        .ord-ship .meta { font-size: 14px; color: var(--muted); }
        .ord-ship .meta code { background: var(--soft); border-radius: 2px; padding: 1px 7px; font-family: var(--serif); font-size: 14px; color: var(--ink); }

        /* ===== detail cards ===== */
        .ord-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 22px; margin-bottom: 22px; }
        .ord-card { background: var(--card); border: 1px solid var(--line); border-radius: 2px; padding: 24px 26px; }
        .ord-card > h3 {
            font-family: var(--display); font-size: 11px; letter-spacing: .14em; text-transform: uppercase; color: var(--muted);
        ...
        .ord-card .row .k { color: var(--muted); font-family: var(--display); font-size: 11px; text-transform: uppercase; letter-spacing: .1em; }
        .ord-card .row .v { color: var(--ink); font-weight: 600; text-align: right; }
        .ord-card .addr { font-size: 15px; line-height: 1.7; color: var(--ink); }

        /* status badge — square chip */
        .ord-badge { display: inline-flex; font-family: var(--display); font-size: 11px; letter-spacing: .08em; text-transform: uppercase; font-weight: 600; padding: 4px 11px; border-radius: 2px; background: var(--soft); color: var(--ink); }
        ...
        .ord-badge.pending { background: #ece1c2; color: #6b5a14; }
        .ord-badge.refunded, .ord-badge.cancelled, .ord-badge.failed { background: #ecd6c8; color: #9c5a3e; }

        /* ===== items + totals ===== */
        .ord-items { background: var(--card); border: 1px solid var(--line); border-radius: 2px; overflow: hidden; }
        .ord-items > h3 { font-family: var(--display); font-size: 11px; letter-spacing: .14em; text-transform: uppercase; color: var(--muted); padding: 20px 26px; border-bottom: 1px solid var(--line); }
        ...
        .ord-item .name { font-family: var(--serif); font-weight: 400; font-size: 18px; line-height: 1.2; color: var(--ink); }
        .ord-item .meta { color: var(--muted); font-size: 13px; margin-top: 3px; }
        .ord-item .pr { font-family: var(--display); font-weight: 600; font-size: 15px; font-variant-numeric: tabular-nums; white-space: nowrap; color: var(--ink); }
        ...
        .ord-tot .row code { background: var(--card); border: 1px solid var(--line); border-radius: 2px; padding: 0 6px; font-family: var(--serif); font-size: 13px; color: var(--ink); }
        .ord-tot .grand { padding-top: 16px; margin-top: 12px; border-top: 1px solid var(--ink); }
        .ord-tot .grand .label { font-family: var(--serif); font-weight: 400; font-size: 18px; color: var(--ink); }
        .ord-tot .grand .num { font-family: var(--body); font-weight: 700; font-size: 27px; font-variant-numeric: tabular-nums; color: var(--ink); }
