...
@section('content')
    ...
    <style>
        ...
        /* numbered field sets */
        .fset { background: none; border-bottom: 1px solid var(--line); padding: 0 0 28px; margin-bottom: 28px; }
        .fset h3 { font-family: var(--display); font-size: 11px; letter-spacing: .14em; text-transform: uppercase; font-weight: 600; margin-bottom: 18px; display: flex; align-items: center; gap: 12px; border-bottom: 1px solid var(--ink); padding-bottom: 12px; }
        .fset h3 .num { font-family: var(--serif); font-style: italic; font-size: 18px; color: var(--accent); }

        .frow { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
        .frow + .frow { margin-top: 16px; }
        .field { display: flex; flex-direction: column; }
        .field.full { grid-column: 1 / -1; }
        .field label { font-family: var(--display); font-size: 10px; font-weight: 600; letter-spacing: .1em; text-transform: uppercase; color: var(--muted); margin-bottom: 7px; }
        .field label small { text-transform: none; letter-spacing: 0; font-weight: 400; }
        .field input, .field select, .field textarea { border: 1px solid var(--line); background: var(--card); border-radius: 2px; padding: 13px 14px; font-family: var(--body); font-size: 14px; color: var(--ink); transition: border-color .2s ease; }
        .field input:focus, .field select:focus, .field textarea:focus { outline: none; border-color: var(--ink); }
        .field textarea { min-height: 90px; resize: vertical; }

        /* signed-in / sign-in banners */
        .co-signed-in { background: var(--soft); border: 1px solid var(--line); border-radius: 2px; padding: 12px 15px; margin-bottom: 18px; font-size: 13px; }
        .co-signin-banner { background: var(--soft); border: 1px solid var(--line); border-radius: 2px; padding: 14px 20px; margin-bottom: 28px; font-size: 13.5px; display: flex; justify-content: space-between; align-items: center; gap: 12px; flex-wrap: wrap; }
        .co-signin-banner a { color: var(--accent); font-weight: 600; border-bottom: 1px solid currentColor; padding-bottom: 1px; }

        .errors { border: 1px solid #9c5a3e; background: color-mix(in srgb, #9c5a3e 10%, var(--card)); border-radius: 2px; padding: 14px 20px; margin-bottom: 26px; font-size: 13.5px; color: var(--ink); }
        .errors ul { padding-left: 20px; }
        .errors li::marker { color: #9c5a3e; }

        /* stub payment notice */
        .stub-notice { background: var(--soft); border: 1px solid var(--line); border-radius: 2px; padding: 16px 20px; font-size: 13.5px; display: flex; gap: 14px; align-items: flex-start; }
        .stub-notice .icon { display: inline-grid; place-items: center; width: 26px; height: 26px; border-radius: 2px; background: var(--ink); color: var(--bg); font-family: var(--display); flex-shrink: 0; }
        .stub-notice strong { display: block; margin-bottom: 4px; font-family: var(--serif); font-weight: 400; }
        .stub-notice em { font-family: var(--serif); font-style: italic; color: var(--accent); }

        /* order summary */
        .osum { background: none; border: 1px solid var(--ink); border-radius: 0; padding: 30px; position: sticky; top: 96px; }
        .osum h2 { font-family: var(--serif); font-weight: 400; font-size: 26px; margin-bottom: 22px; }
        .osum h2 em { font-style: italic; color: var(--accent); }
        .osum .oitem { display: grid; grid-template-columns: 54px 1fr auto; gap: 14px; align-items: center; margin-bottom: 18px; }
        .osum .oitem .img { height: 60px; overflow: hidden; position: relative; background: var(--stone); }
        .osum .oitem .img img { width: 100%; height: 100%; object-fit: cover; }
        .osum .oitem .qty-pill { position: absolute; top: -8px; right: -8px; background: var(--ink); color: var(--bg); width: 22px; height: 22px; border-radius: 2px; display: grid; place-items: center; font-family: var(--display); font-size: 11px; }
        .osum .oitem .nm { font-size: 14px; line-height: 1.25; }
        .osum .oitem .m { font-size: 12px; color: var(--muted); }
        .osum .oitem .pr { font-family: var(--display); font-weight: 600; font-size: 14px; font-variant-numeric: tabular-nums; color: var(--ink); }
        .osum .divider { border: 0; border-top: 1px solid var(--line); margin: 18px 0; }
        .osum .r { display: flex; justify-content: space-between; font-size: 14px; margin: 12px 0; color: #5d5c50; }
        .osum .r.discount { color: var(--accent); }
        .osum .tot { display: flex; justify-content: space-between; align-items: baseline; font-size: 18px; font-weight: 600; border-top: 1px solid var(--ink); padding-top: 16px; margin: 14px 0 18px; }
        .osum .tot span:last-child { font-family: var(--body); font-weight: 700; font-variant-numeric: tabular-nums; color: var(--ink); }
        ...
        .wz-steps li { display: flex; align-items: center; gap: 11px; flex: 1; font-family: var(--display); font-size: 11px; letter-spacing: .1em; text-transform: uppercase; font-weight: 600; color: var(--muted); }
        .wz-steps li:not(:last-child)::after { content: ""; flex: 1; height: 1px; background: var(--line); margin: 0 14px; }
        .wz-steps .dot { width: 34px; height: 34px; border: 1px solid var(--line); border-radius: 2px; display: grid; place-items: center; font-family: var(--serif); font-style: italic; font-size: 16px; color: var(--ink); background: var(--card); flex-shrink: 0; transition: background-color .2s ease, color .2s ease, border-color .2s ease; }
        .wz-steps li.is-current .dot { background: var(--accent); color: var(--bg); border-color: var(--accent); }
        .wz-steps li.is-current { color: var(--ink); }
        .wz-steps li.is-done { color: var(--ink); cursor: pointer; }
        .wz-steps li.is-done .dot { background: var(--ink); color: var(--bg); border-color: var(--ink); font-style: normal; }
        ...
        .wz-back { background: transparent; border: 1px solid var(--ink); border-radius: 2px; color: var(--ink); font-family: var(--display); font-size: 12px; font-weight: 600; letter-spacing: .12em; text-transform: uppercase; padding: 13px 24px; transition: background-color .25s ease, color .25s ease; }
        .wz-back:hover { background: var(--ink); color: var(--bg); }
        .pay-btn { margin-left: auto; display: inline-flex; align-items: baseline; gap: 8px; background: var(--ink); color: var(--bg); border: 1px solid var(--ink); border-radius: 2px; padding: 15px 34px; font-family: var(--display); font-size: 12px; font-weight: 600; letter-spacing: .12em; text-transform: uppercase; transition: background-color .25s ease, color .25s ease; }
        .pay-btn:hover { background: transparent; color: var(--ink); }
        .pay-btn:disabled { opacity: .5; cursor: not-allowed; background: var(--ink); color: var(--bg); }
