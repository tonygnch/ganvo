...
@section('content')
    <style>
        ...
        .ord-hero .mark {
            ...
            border-radius: 2px; background: var(--accent); color: #fdf8ef;
            font-size: 32px; line-height: 1; box-shadow: 6px 6px 0 var(--soft2);
        }
        .ord-hero .mark.danger { background: #9c5a3e; }
        .ord-hero h1 { font-family: var(--display); font-weight: 700; font-size: clamp(34px, 5vw, 56px); line-height: 1.02; margin-bottom: 14px; }
        ...
        .ord-hero .num {
            ...
            font-family: var(--mono); font-size: 13px; letter-spacing: .04em; text-transform: uppercase; color: var(--accent);
            padding: 9px 18px;
            /* stamped order number — the roaster's mark on the receipt */
            transform: rotate(-1.4deg);
            -webkit-mask-image: var(--stamp); mask-image: var(--stamp);
        ...
        .ord-ship .ic { width: 48px; height: 48px; border-radius: 2px; background: var(--accent); color: #fdf8ef; display: grid; place-items: center; font-size: 22px; flex-shrink: 0; }
        .ord-ship .tx { flex: 1; min-width: 200px; }
        .ord-ship h3 { font-family: var(--display); font-weight: 600; font-size: 18px; margin-bottom: 4px; color: var(--ink); }
        .ord-ship .meta { font-size: 14px; color: var(--muted); }
        .ord-ship .meta code { background: var(--soft); border-radius: 3px; padding: 1px 7px; font-family: var(--mono); font-size: 13px; color: var(--ink); }

        /* ===== detail cards ===== */
        .ord-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 22px; margin-bottom: 22px; }
        .ord-card { background: var(--card); border: 1.5px solid var(--ink); border-radius: 4px; padding: 24px 26px; }
        .ord-card > h3 {
            font-family: var(--display); font-weight: 700; font-size: 20px; color: var(--ink);
            margin-bottom: 16px; padding-bottom: 14px; border-bottom: 1.5px solid var(--ink);
        }
        ...
        .ord-card .row .k { color: var(--muted); font-family: var(--mono); font-size: 11px; text-transform: uppercase; letter-spacing: .04em; }
        .ord-card .row .v { color: var(--ink); font-weight: 600; text-align: right; }
        .ord-card .addr { font-size: 15px; line-height: 1.7; color: var(--ink); }

        /* status badge — squared chip */
        .ord-badge { display: inline-flex; font-family: var(--mono); font-size: 11px; letter-spacing: .04em; text-transform: uppercase; padding: 4px 11px; border-radius: 2px; background: var(--soft); color: var(--ink); }
        ...
        /* ===== items + totals ===== */
        .ord-items { background: var(--card); border: 1.5px solid var(--ink); border-radius: 4px; overflow: hidden; box-shadow: 6px 6px 0 var(--soft2); }
        .ord-items > h3 { font-family: var(--display); font-weight: 700; font-size: 20px; color: var(--ink); padding: 20px 26px; border-bottom: 1.5px solid var(--ink); }
        .ord-items > h3::before { content: "✦ "; color: var(--accent); }
        .ord-item { display: grid; grid-template-columns: 1fr auto; gap: 16px; align-items: center; padding: 16px 26px; border-bottom: 1px solid var(--rule); }
        .ord-item .name { font-family: var(--display); font-weight: 600; font-size: 18px; line-height: 1.2; color: var(--ink); }
        .ord-item .meta { color: var(--muted); font-family: var(--mono); font-size: 11px; margin-top: 3px; text-transform: uppercase; letter-spacing: .04em; }
        .ord-item .pr { font-family: var(--display); font-weight: 600; font-size: 18px; font-variant-numeric: tabular-nums; white-space: nowrap; color: var(--accent); }

        .ord-tot { padding: 20px 26px 24px; background: var(--soft); border-top: 1.5px solid var(--ink); }
        .ord-tot .row { display: flex; justify-content: space-between; gap: 12px; padding: 6px 0; font-size: 14px; color: var(--muted); }
        .ord-tot .row .num { font-weight: 600; color: var(--ink); font-family: var(--display); font-variant-numeric: tabular-nums; font-size: 16px; }
        .ord-tot .row.disc .num { color: var(--accent); }
        .ord-tot .row code { background: var(--card); border: 1px solid var(--line); border-radius: 3px; padding: 0 6px; font-family: var(--mono); font-size: 12px; color: var(--ink); }
        .ord-tot .grand { padding-top: 16px; margin-top: 12px; border-top: 1.5px solid var(--ink); }
        .ord-tot .grand .label { font-family: var(--display); font-weight: 700; font-size: 18px; color: var(--ink); }
        .ord-tot .grand .num { font-family: var(--display); font-weight: 700; font-size: 27px; font-variant-numeric: tabular-nums; color: var(--accent); }
