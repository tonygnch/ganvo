...
@section('content')
    ...
    <style>
        /* Ember PDP — sticky bordered gallery on the left, spec-sheet info
           column on the right. Hard ink outlines, Spectral accent price. */
        .pdp { display: grid; grid-template-columns: 1fr 1fr; gap: 50px; padding: 20px 0 0; align-items: start; }

        /* gallery — sticky hero + thumbnail row */
        .pgal { position: sticky; top: calc(var(--header-height) + 24px); }
        .pgal .main { height: 520px; border: 1.5px solid var(--ink); border-radius: 4px; overflow: hidden; position: relative; box-shadow: 6px 6px 0 var(--soft2); }
        .pgal .main[data-pdp-open] { cursor: zoom-in; }
        .pgal .main img { width: 100%; height: 100%; object-fit: cover; transition: transform .6s cubic-bezier(.19, .7, .16, 1); }
        .pgal .main[data-pdp-open]:hover img { transform: scale(1.025); }
        @media (prefers-reduced-motion: reduce) { .pgal .main img, .pgal .main[data-pdp-open]:hover img { transform: none; transition: none; } }
        .pgal .thumbs { display: flex; gap: 12px; margin-top: 12px; }
        .pgal .thumbs .pdp-thumb { flex: 1; height: 84px; border: 1.5px solid var(--line); border-radius: 3px; cursor: pointer; opacity: .6; overflow: hidden; padding: 0; background: var(--soft); transition: opacity .2s ease, border-color .2s ease; }
        ...
        .pgal .thumbs .pdp-thumb.on { border-color: var(--ink); }

        /* info column */
        .pinfo .crumb { font-family: var(--mono); font-size: 12px; letter-spacing: .04em; text-transform: uppercase; color: var(--muted); margin-bottom: 14px; }
        .pinfo .crumb a:hover { color: var(--accent); }
        /* origin label — stamped chip, echoing the board's schedule stamp */
        .pinfo .cat { display: inline-block; font-family: var(--mono); font-size: 11px; letter-spacing: .06em; text-transform: uppercase; color: var(--accent); border: 1.5px solid var(--accent); border-radius: 2px; padding: 4px 10px; transform: rotate(-1.4deg); -webkit-mask-image: var(--stamp); mask-image: var(--stamp); }
        .pinfo h1 { font-family: var(--display); font-weight: 700; font-size: clamp(36px, 4.4vw, 56px); line-height: 1.02; letter-spacing: -.018em; margin: 8px 0 10px; }
        .pinfo h1 em { font-style: italic; color: var(--accent); }
        .pinfo .price { font-family: var(--display); font-weight: 600; font-size: 28px; font-variant-numeric: tabular-nums; margin: 16px 0; color: var(--accent); }
        .pinfo .stock { font-family: var(--mono); font-size: 12px; text-transform: uppercase; letter-spacing: .04em; color: var(--muted); margin: -6px 0 18px; display: inline-flex; align-items: center; gap: 8px; }
        .pinfo .stock .dot { width: 8px; height: 8px; border-radius: 99px; background: var(--accent); box-shadow: 0 0 0 0 color-mix(in srgb, var(--accent) 45%, transparent); animation: stockPulse 2.6s ease-out infinite; }
        @keyframes stockPulse { 0% { box-shadow: 0 0 0 0 color-mix(in srgb, var(--accent) 45%, transparent); } 60% { box-shadow: 0 0 0 7px transparent; } 100% { box-shadow: 0 0 0 0 transparent; } }
        ...
        .pinfo ul.perks { list-style: none; border-top: 1.5px solid var(--ink); border-bottom: 1.5px solid var(--ink); margin-bottom: 24px; }
        .pinfo ul.perks li { padding: 12px 0; border-bottom: 1px solid var(--rule); font-size: 14px; display: flex; gap: 10px; }
        .pinfo ul.perks li:last-child { border-bottom: none; }
        .pinfo ul.perks li::before { content: "✦"; color: var(--accent); }

        .opt { margin-bottom: 20px; }
        .opt .ol { font-family: var(--mono); font-size: 11px; letter-spacing: .06em; text-transform: uppercase; color: var(--muted); margin-bottom: 10px; }

        /* ===== Fullscreen image viewer (lightbox) — Ember styling.
           Esc closes, ←/→ navigate, focus is trapped while open. ===== */
        .pdp-lightbox { position: fixed; inset: 0; z-index: 200; background: rgba(34, 21, 13, .9); backdrop-filter: blur(6px); -webkit-backdrop-filter: blur(6px); display: flex; align-items: center; justify-content: center; padding: 6vh 6vw; animation: lbFade .22s ease; }
        ...
        .pdp-lightbox #pdp-lightbox-image { max-width: 86vw; max-height: 80vh; object-fit: contain; border-radius: 4px; background: var(--card); padding: 10px; box-shadow: 0 30px 70px -26px rgba(0, 0, 0, .65); }
        .pdp-lightbox .lb-count { font-family: var(--mono); font-size: 13px; text-transform: uppercase; letter-spacing: .06em; color: var(--soft); }
        .pdp-lightbox .lb-close { position: absolute; top: 24px; right: 26px; width: 50px; height: 50px; border-radius: 2px; background: var(--card); color: var(--ink); border: 1.5px solid var(--ink); font-size: 17px; line-height: 1; display: grid; place-items: center; transition: background-color .22s ease, color .22s ease; }
        .pdp-lightbox .lb-close:hover { background: var(--accent); color: #fdf8ef; border-color: var(--accent); }
        .pdp-lightbox .lb-nav { position: absolute; top: 50%; transform: translateY(-50%); width: 56px; height: 56px; border-radius: 2px; background: var(--card); color: var(--ink); border: 1.5px solid var(--ink); font-family: var(--display); font-size: 30px; line-height: 1; display: grid; place-items: center; transition: background-color .22s ease, color .22s ease; }
        .pdp-lightbox .lb-prev { left: 24px; }
        .pdp-lightbox .lb-next { right: 24px; }
        .pdp-lightbox .lb-nav:hover { background: var(--accent); color: #fdf8ef; border-color: var(--accent); }
        ...
        <div class="wrap" style="padding-top: 24px;">
            <div class="pdp">
                ...
                <div class="pinfo">
                    ...
                    <form method="post" action="/cart/add/{{ $product->slug }}">
                        ...
                        <button type="submit" class="btn block accent" data-vp-submit @if ($product->hasVariants()) disabled @endif>
            ...
            @if ($related->isNotEmpty())
                <div class="sec-head" style="margin-top: 70px;">
                    <span class="kicker">{{ __('site.storefront.ember.related_eyebrow') }}</span>
                    <h2>{{ __('site.storefront.featured.browse_all') }}</h2>
                    <a class="more" href="/">{{ __('site.storefront.featured.browse_all') }} →</a>
                </div>
                <div class="blooms">
                    @foreach ($related as $rp)
                        @include('themes.ember._card', ['product' => $rp, 'badge' => null])
                    @endforeach
                </div>
            @endif
        </div>
