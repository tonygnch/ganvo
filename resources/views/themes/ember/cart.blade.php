...
@section('content')
    ...
    <style>
        ...
        .cart-empty { background: var(--card); border: 1.5px solid var(--ink); border-radius: 4px; box-shadow: 8px 8px 0 var(--soft2); text-align: center; padding: 70px 28px; margin-top: 26px; position: relative; }
        .cart-empty h2 { font-family: var(--display); font-size: 30px; font-weight: 700; margin-bottom: 10px; }
        .cart-empty h2 em { font-style: italic; color: var(--accent); }
        .cart-empty p { color: var(--muted); margin-bottom: 26px; }

        .cart { display: grid; grid-template-columns: 1fr 360px; gap: 48px; padding: 20px 0 0; align-items: start; }

        .lines { border-top: 1.5px solid var(--ink); }
        .line { display: grid; grid-template-columns: 84px 1fr auto; gap: 18px; padding: 22px 0; border-bottom: 1px solid var(--rule); align-items: center; }
        .line .img { height: 84px; width: 84px; border: 1.5px solid var(--line); border-radius: 3px; overflow: hidden; flex-shrink: 0; }
        .line .img img { width: 100%; height: 100%; object-fit: cover; }
        .line .img .ph, .line .img .bloomph { display: grid; place-items: center; }
        .line .t { font-family: var(--display); font-weight: 600; font-size: 18px; line-height: 1.2; }
        .line .t a:hover { color: var(--accent); }
        .line .m { font-family: var(--mono); font-size: 11px; color: var(--muted); margin-top: 2px; text-transform: uppercase; letter-spacing: .04em; }
        .line .unit { font-family: var(--mono); font-size: 11px; color: var(--muted); margin-top: 4px; }

        .qty { display: inline-flex; border: 1.5px solid var(--ink); border-radius: 2px; margin-top: 10px; overflow: hidden; }
        .qty form { display: inline-flex; }
        .qty button { width: 30px; height: 30px; background: none; border: none; font-size: 15px; color: var(--ink); transition: background-color .2s ease; }
        .qty button:hover { background: var(--soft); }
        .qty .n { width: 34px; display: grid; place-items: center; font-size: 13px; border-left: 1px solid var(--rule); border-right: 1px solid var(--rule); }

        .line .actions { text-align: right; }
        .line .pr { font-family: var(--display); font-weight: 600; font-size: 18px; font-variant-numeric: tabular-nums; text-align: right; color: var(--accent); }
        .line .rm { font-family: var(--mono); font-size: 11px; text-transform: uppercase; letter-spacing: .04em; color: var(--muted); background: none; border: none; display: block; margin-top: 8px; margin-left: auto; transition: color .2s ease; }
        .line .rm:hover { color: var(--ink); text-decoration: underline; }

        .cart-actions { margin-top: 24px; }

        .summary { background: var(--card); border: 1.5px solid var(--ink); border-radius: 4px; padding: 30px; position: sticky; top: 96px; box-shadow: 6px 6px 0 var(--soft2); }
        .summary h3 { font-family: var(--display); font-size: 22px; font-weight: 700; margin-bottom: 20px; }

        .promo-region { margin-bottom: 18px; }
        .summary .promo { display: flex; border: 1.5px solid var(--ink); border-radius: 2px; overflow: hidden; }
        .summary .promo input { flex: 1; border: none; padding: 11px 14px; font-family: var(--mono); font-size: 12px; background: var(--bg); color: var(--ink); min-width: 0; }
        .summary .promo input:focus { outline: none; }
        .summary .promo .btn { border-radius: 0; padding: 0 16px; font-size: 11px; }
        .summary .applied { padding: 11px 14px; background: var(--soft); border: 1.5px solid var(--ink); border-radius: 2px; font-size: 13px; display: flex; justify-content: space-between; align-items: center; gap: 8px; }
        .summary .applied .code { font-weight: 600; color: var(--accent); }
        .summary .applied form button { background: none; border: none; font-family: var(--mono); font-size: 11px; text-transform: uppercase; letter-spacing: .04em; color: var(--muted); text-decoration: underline; }
        ...
        .summary .r small { font-family: var(--mono); font-size: 11px; text-transform: uppercase; letter-spacing: .04em; }
        .summary .r.discount { color: var(--accent); }
        .summary .tot { display: flex; justify-content: space-between; font-size: 18px; font-weight: 700; color: var(--ink); border-top: 1.5px solid var(--ink); padding-top: 16px; margin: 8px 0 20px; }
        .summary .tot span:last-child { font-family: var(--display); font-variant-numeric: tabular-nums; color: var(--accent); }
        .summary .secure { margin-top: 16px; text-align: center; font-family: var(--mono); font-size: 11px; text-transform: uppercase; letter-spacing: .04em; color: var(--muted); }
            ...
            @else
                <div class="cart" data-cart-root data-num-anim="{{ $store->numberAnimation() }}">
                    ...
                    <aside class="summary reveal s1">
                        ...
                        <a class="btn block accent" href="/checkout">{{ __('site.cart.checkout') }}</a>
