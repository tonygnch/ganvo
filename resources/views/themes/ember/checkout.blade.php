...
@section('content')
    ...
    <style>
        ...
        /* numbered field sets */
        .fset { background: var(--card); border: 1.5px solid var(--ink); border-radius: 4px; padding: 26px 28px; margin-bottom: 22px; }
        .fset h3 { font-family: var(--display); font-size: 19px; font-weight: 700; margin-bottom: 20px; display: flex; align-items: center; gap: 14px; }
        .fset h3 .num { background: var(--accent); color: #fdf8ef; width: 30px; height: 30px; border-radius: 2px; display: grid; place-items: center; font-family: var(--display); font-size: 15px; flex-shrink: 0; }

        .frow { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
        .frow + .frow { margin-top: 16px; }
        .field { display: flex; flex-direction: column; }
        .field.full { grid-column: 1 / -1; }
        .field label { font-family: var(--mono); font-size: 11px; letter-spacing: .04em; text-transform: uppercase; color: var(--muted); margin-bottom: 8px; }
        .field label small { text-transform: none; letter-spacing: 0; }
        .field input, .field select, .field textarea { border: 1.5px solid var(--line); background: var(--card); border-radius: 2px; padding: 12px 14px; font-family: var(--body); font-size: 14px; color: var(--ink); transition: border-color .2s ease; }
        .field input:focus, .field select:focus, .field textarea:focus { outline: none; border-color: var(--ink); }
        .field textarea { min-height: 90px; resize: vertical; }

        /* signed-in / sign-in banners */
        .co-signed-in { background: color-mix(in srgb, var(--accent) 12%, var(--card)); border: 1.5px solid var(--line); border-radius: 2px; padding: 12px 15px; margin-bottom: 18px; font-size: 13px; }
        .co-signin-banner { background: var(--soft); border: 1.5px solid var(--ink); border-radius: 3px; padding: 14px 20px; margin-bottom: 28px; font-size: 13.5px; display: flex; justify-content: space-between; align-items: center; gap: 12px; flex-wrap: wrap; }
        .co-signin-banner a { color: var(--accent); font-weight: 600; border-bottom: 1px solid currentColor; padding-bottom: 1px; }

        .errors { border: 1.5px solid #c0573e; background: color-mix(in srgb, #c0573e 10%, var(--card)); border-radius: 3px; padding: 14px 20px; margin-bottom: 26px; font-size: 13.5px; color: var(--ink); }
        .errors ul { padding-left: 20px; }
        .errors li::marker { color: #c0573e; }

        /* stub payment notice */
        .stub-notice { background: color-mix(in srgb, var(--accent) 10%, var(--card)); border: 1.5px solid var(--line); border-radius: 3px; padding: 16px 20px; font-size: 13.5px; display: flex; gap: 14px; align-items: flex-start; }
        .stub-notice .icon { display: inline-grid; place-items: center; width: 26px; height: 26px; border-radius: 2px; background: var(--accent); color: #fdf8ef; font-family: var(--display); flex-shrink: 0; }
        .stub-notice strong { display: block; margin-bottom: 4px; font-family: var(--display); font-weight: 700; }
        .stub-notice em { font-style: italic; color: var(--accent); }

        /* order summary */
        .osum { background: var(--card); border: 1.5px solid var(--ink); border-radius: 4px; padding: 28px; position: sticky; top: 96px; box-shadow: 6px 6px 0 var(--soft2); }
        .osum h2 { font-family: var(--display); font-weight: 700; font-size: 26px; margin-bottom: 22px; }
        .osum h2 em { font-style: italic; color: var(--accent); }
        .osum .oitem { display: grid; grid-template-columns: 56px 1fr auto; gap: 14px; align-items: center; margin-bottom: 18px; }
        .osum .oitem .img { height: 64px; border: 1.5px solid var(--line); border-radius: 3px; overflow: hidden; position: relative; }
        .osum .oitem .img img { width: 100%; height: 100%; object-fit: cover; }
        .osum .oitem .qty-pill { position: absolute; top: -8px; right: -8px; background: var(--accent); color: #fdf8ef; width: 22px; height: 22px; border-radius: 2px; display: grid; place-items: center; font-family: var(--display); font-size: 11px; }
        .osum .oitem .nm { font-family: var(--display); font-weight: 600; font-size: 15px; line-height: 1.25; }
        .osum .oitem .m { font-family: var(--mono); font-size: 11px; color: var(--muted); }
        .osum .oitem .pr { font-family: var(--display); font-weight: 600; font-size: 17px; font-variant-numeric: tabular-nums; color: var(--accent); }
        .osum .divider { border: 0; border-top: 1.5px solid var(--ink); margin: 18px 0; }
        ...
        .osum .tot { display: flex; justify-content: space-between; align-items: baseline; font-family: var(--display); font-weight: 700; font-size: 21px; border-top: 1.5px solid var(--ink); padding-top: 18px; margin: 16px 0 18px; }
        .osum .tot span:last-child { font-variant-numeric: tabular-nums; color: var(--accent); }
        .osum .secure { font-family: var(--mono); font-size: 11px; text-transform: uppercase; letter-spacing: .04em; color: var(--muted); text-align: center; margin-top: 12px; }

        /* wizard stepper */
        .wz-steps { display: none; }
        .wz-on .wz-steps { display: flex; align-items: center; gap: 0; list-style: none; margin: 0 0 34px; padding: 0; }
        .wz-steps li { display: flex; align-items: center; gap: 11px; flex: 1; font-family: var(--mono); font-size: 12px; letter-spacing: .04em; text-transform: uppercase; color: var(--muted); }
        .wz-steps li:not(:last-child)::after { content: ""; flex: 1; height: 1px; background: var(--line); margin: 0 14px; }
        .wz-steps .dot { width: 34px; height: 34px; border: 1.5px solid var(--line); border-radius: 2px; display: grid; place-items: center; font-family: var(--display); font-size: 14px; color: var(--ink); background: var(--card); flex-shrink: 0; transition: background-color .2s ease, color .2s ease, border-color .2s ease; }
        .wz-steps li.is-current .dot { background: var(--accent); color: #fdf8ef; border-color: var(--accent); }
        ...
        .wz-back { background: transparent; border: 1.5px solid var(--ink); border-radius: 2px; color: var(--ink); font-family: var(--mono); font-size: 12px; font-weight: 700; text-transform: uppercase; padding: 13px 24px; transition: background-color .22s ease, color .22s ease; }
        .wz-back:hover { background: var(--ink); color: var(--bg); }
        .pay-btn { margin-left: auto; display: inline-flex; align-items: baseline; gap: 8px; background: var(--accent); color: #fdf8ef; border: 1.5px solid var(--accent); border-radius: 2px; padding: 15px 34px; font-family: var(--mono); font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: .04em; transition: background-color .22s ease, color .22s ease; }
        .pay-btn:hover { background: transparent; color: var(--accent); }
        .pay-btn:disabled { opacity: .55; cursor: not-allowed; background: var(--accent); color: #fdf8ef; }
