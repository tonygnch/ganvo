...
@section('content')
    <style>
        /* ===== Category — filter rail + Forma grid ===== */
        .catalog { display: grid; grid-template-columns: 250px 1fr; gap: 40px; align-items: start; padding-top: 30px; }

        /* decorative pills (design accent — not functional) */
        .pills { display: flex; flex-wrap: wrap; gap: 10px; margin: 30px 0 8px; }
        .pills span {
            font-size: 13px; font-weight: 600;
            color: var(--ink); background: var(--card); border: 1px solid var(--line);
            padding: 10px 18px; border-radius: 99px;
        }
        .pills span.on { color: #fff; background: var(--ink); border-color: var(--ink); }
        ...
        .filters select {
            padding: 11px 13px; background: var(--bg); border: 1px solid var(--line2);
        ...
        .filters .clear {
            ...
            align-self: center; transition: color .2s ease;
        }
        .filters .clear:hover { color: var(--accent); }

        /* result count line — mono spec label */
        .toolbar {
            font-family: var(--mono); font-size: 12px; color: var(--muted);
            margin: 30px 0 22px; display: flex; align-items: center; gap: 10px;
        }
