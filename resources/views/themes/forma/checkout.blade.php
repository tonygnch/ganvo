...
@section('content')
    ...
    <style>
        ...
        /* numbered field sets */
        .fset { background: var(--card); border: 1px solid var(--line); border-radius: 16px; padding: 26px; margin-bottom: 16px; }
        .fset h3 { font-family: var(--mono); font-size: 12px; text-transform: uppercase; margin-bottom: 18px; display: flex; align-items: center; gap: 12px; color: var(--ink); }
        .fset h3 .num { background: var(--accent); color: #fff; width: 24px; height: 24px; border-radius: 7px; display: grid; place-items: center; font-family: var(--display); font-size: 12px; font-weight: 700; flex-shrink: 0; }
        ...
        .field input, .field select, .field textarea { border: 1px solid var(--line2); background: var(--bg); border-radius: 10px; padding: 13px 15px; font-family: var(--body); font-size: 14px; color: var(--ink); transition: border-color .2s ease; }
        .field input:focus, .field select:focus, .field textarea:focus { outline: none; border-color: var(--accent); }
        .field textarea { min-height: 90px; resize: vertical; }

        /* signed-in / sign-in banners */
        .co-signed-in { background: color-mix(in srgb, var(--accent) 10%, var(--card)); border: 1px solid var(--line); border-radius: 10px; padding: 12px 15px; margin-bottom: 18px; font-size: 13px; }
        ...
        .errors { border: 1px solid #c0654a; background: color-mix(in srgb, #c0654a 10%, var(--card)); border-radius: 12px; padding: 14px 20px; margin-bottom: 26px; font-size: 13.5px; color: var(--ink); }
        .errors ul { padding-left: 20px; }
        .errors li::marker { color: #c0654a; }

        /* stub payment notice */
        .stub-notice { background: color-mix(in srgb, var(--accent) 8%, var(--card)); border: 1px solid var(--line); border-radius: 12px; padding: 16px 20px; font-size: 13.5px; display: flex; gap: 14px; align-items: flex-start; }
        .stub-notice .icon { display: inline-grid; place-items: center; width: 26px; height: 26px; border-radius: 8px; background: var(--accent); color: #fff; font-family: var(--display); font-weight: 700; flex-shrink: 0; }
        .stub-notice strong { display: block; margin-bottom: 4px; font-family: var(--display); font-weight: 700; }
        .stub-notice em { font-style: normal; font-weight: 700; color: var(--accent); }

        /* order summary */
        .osum { background: var(--card); border: 1px solid var(--line); border-radius: 16px; padding: 28px; position: sticky; top: 90px; }
        .osum h2 { font-family: var(--display); font-weight: 700; font-size: 21px; margin-bottom: 22px; }
        .osum h2 em { font-style: normal; color: var(--accent); }
        .osum .oitem { display: grid; grid-template-columns: 50px 1fr auto; gap: 14px; align-items: center; margin-bottom: 16px; }
        .osum .oitem .img { height: 56px; border-radius: 10px; overflow: hidden; position: relative; }
        .osum .oitem .img img { width: 100%; height: 100%; object-fit: cover; }
        .osum .oitem .qty-pill { position: absolute; top: -8px; right: -8px; background: var(--accent); color: #fff; width: 22px; height: 22px; border-radius: 99px; display: grid; place-items: center; font-family: var(--display); font-size: 11px; font-weight: 700; box-shadow: 0 2px 6px rgba(20, 22, 28, .25); }
        .osum .oitem .nm { font-size: 14px; font-weight: 600; line-height: 1.25; }
        .osum .oitem .m { font-size: 12px; color: var(--muted); }
        .osum .oitem .pr { font-family: var(--display); font-weight: 600; font-size: 15px; font-variant-numeric: tabular-nums; }
        ...
        .osum .tot { display: flex; justify-content: space-between; align-items: baseline; font-size: 18px; font-weight: 700; border-top: 1px solid var(--line); padding-top: 15px; margin: 14px 0 20px; }
        .osum .tot span:last-child { font-family: var(--display); font-weight: 700; font-variant-numeric: tabular-nums; }
        .osum .secure { font-family: var(--mono); font-size: 11px; color: var(--muted); text-align: center; margin-top: 12px; }

        /* wizard stepper */
        .wz-steps { display: none; }
        .wz-on .wz-steps { display: flex; align-items: center; gap: 0; list-style: none; margin: 0 0 34px; padding: 0; }
        .wz-steps li { display: flex; align-items: center; gap: 11px; flex: 1; font-family: var(--mono); font-size: 11px; text-transform: uppercase; color: var(--muted); }
        .wz-steps li:not(:last-child)::after { content: ""; flex: 1; height: 1px; background: var(--line); margin: 0 14px; }
        .wz-steps .dot { width: 34px; height: 34px; border: 1px solid var(--line2); border-radius: 99px; display: grid; place-items: center; font-family: var(--display); font-size: 14px; font-weight: 700; color: var(--ink); background: var(--card); flex-shrink: 0; transition: background-color .2s ease, color .2s ease, border-color .2s ease; }
        .wz-steps li.is-current .dot { background: var(--accent); color: #fff; border-color: var(--accent); }
        ...
        .wz-back { background: transparent; border: 1px solid var(--line2); border-radius: 10px; color: var(--ink); font-size: 13px; font-weight: 600; padding: 13px 24px; transition: border-color .25s ease; }
        .wz-back:hover { border-color: var(--ink); }
        .pay-btn { margin-left: auto; display: inline-flex; align-items: baseline; gap: 8px; background: var(--accent); color: #fff; border: 1px solid var(--accent); border-radius: 10px; padding: 15px 34px; font-size: 14px; font-weight: 600; transition: filter .18s ease, transform .18s ease; }
