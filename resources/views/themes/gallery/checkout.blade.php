...
@section('content')
    <style>
        .co-wrap { padding: 34px 0 60px; } .co-wrap > h1 { font-family: var(--display); font-weight: 700; font-size: 40px; letter-spacing: -.02em; margin-bottom: 24px; }
        .wz-steps { display: none; } .wz-on .wz-steps { display: flex; gap: 0; list-style: none; margin: 0 0 28px; padding: 0; }
        .wz-steps li { display: flex; align-items: center; gap: 10px; flex: 1; font-size: 12px; letter-spacing: .06em; text-transform: uppercase; color: var(--muted); }
        .wz-steps li:not(:last-child)::after { content: ""; flex: 1; height: 1px; background: var(--line); margin: 0 12px; }
        .wz-steps .dot { width: 27px; height: 27px; border: 1px solid var(--line); border-radius: 50%; display: grid; place-items: center; font-family: var(--display); flex-shrink: 0; }
        .wz-steps li.is-current { color: var(--ink); } .wz-steps li.is-current .dot { border-color: var(--accent); color: var(--accent); }
        .wz-steps li.is-done { color: var(--ink); cursor: pointer; } .wz-steps li.is-done .dot { background: var(--accent); color: #faf7f1; border-color: var(--accent); }
        .checkout { display: grid; grid-template-columns: 1fr 400px; gap: 54px; align-items: start; }
        .wz-on .wz-step { display: none; } .wz-on .wz-step.is-current { display: block; animation: fade .3s ease; }
        @keyframes fade { from { opacity: 0; transform: translateY(8px); } }
        .fset { margin-bottom: 32px; }
        .fset h3 { font-size: 13px; letter-spacing: .06em; text-transform: uppercase; margin-bottom: 18px; display: flex; align-items: center; gap: 12px; }
        .fset h3 .num { width: 25px; height: 25px; background: var(--accent); color: #faf7f1; border-radius: 50%; display: grid; place-items: center; font-family: var(--display); font-size: 13px; }
        ...
        .field input, .field select, .field textarea { border: 1px solid var(--line); border-radius: 8px; background: var(--card); padding: 13px 15px; font-family: inherit; font-size: 14px; color: var(--ink); }
        .field input:focus, .field select:focus, .field textarea:focus { outline: none; border-color: var(--accent); }
        .co-signed-in { background: var(--soft); border-radius: 10px; padding: 12px 14px; margin-bottom: 14px; font-size: 13px; }
        .co-signin-banner { background: var(--card); border: 1px solid var(--line); border-radius: 12px; padding: 14px 18px; margin-bottom: 24px; font-size: 13px; display: flex; justify-content: space-between; gap: 12px; flex-wrap: wrap; } .co-signin-banner a { color: var(--accent); font-weight: 600; }
        .errors { border: 1px solid #c2705a; background: #f3e2da; color: #8a3f2c; padding: 14px 18px; border-radius: 10px; margin-bottom: 22px; font-size: 13px; } .errors ul { padding-left: 18px; }
        .stub-notice { background: var(--soft); border-radius: 10px; padding: 14px 18px; font-size: 13px; display: flex; gap: 12px; } .stub-notice .icon { color: var(--accent); font-weight: 700; }
        .wz-actions { display: flex; align-items: center; gap: 12px; }
        .wz-back { background: none; border: 1px solid var(--line); color: var(--muted); border-radius: 8px; padding: 13px 24px; font-size: 13px; font-weight: 600; cursor: pointer; } .wz-back:hover { border-color: var(--ink); color: var(--ink); }
        .pay-btn { margin-left: auto; display: inline-flex; align-items: center; gap: 8px; background: var(--accent); color: #faf7f1; border: 0; border-radius: 8px; padding: 15px 32px; font-weight: 600; font-size: 14px; cursor: pointer; font-family: var(--body); } .pay-btn:hover { filter: brightness(1.05); }
        .osum { background: var(--card); border: 1px solid var(--line); border-radius: 16px; padding: 30px; position: sticky; top: 100px; }
        .osum h2 { font-family: var(--display); font-weight: 600; font-size: 22px; margin-bottom: 20px; }
        .osum .oitem { display: grid; grid-template-columns: 54px 1fr auto; gap: 14px; align-items: center; margin-bottom: 16px; }
        .osum .oitem .img { height: 54px; width: 54px; background: var(--soft); border-radius: 10px; overflow: hidden; position: relative; } .osum .oitem .img img { width: 100%; height: 100%; object-fit: cover; }
        .osum .oitem .qty-pill { position: absolute; top: -6px; right: -6px; background: var(--accent); color: #faf7f1; width: 20px; height: 20px; border-radius: 50%; font-size: 11px; display: grid; place-items: center; }
        .osum .oitem .nm { font-size: 14px; font-weight: 600; } .osum .oitem .m { font-size: 12px; color: var(--muted); } .osum .oitem .pr { font-family: var(--display); font-weight: 600; }
        ...
        .osum .tot { display: flex; justify-content: space-between; font-size: 18px; font-weight: 700; border-top: 1px solid var(--line); padding-top: 15px; margin: 14px 0 18px; } .osum .tot [data-sm-grand] { font-family: var(--display); color: var(--accent); }
