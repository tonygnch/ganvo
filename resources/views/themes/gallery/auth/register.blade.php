@php
    ...
    $reqMarker = fn (bool $req) => $req ? '' : ' <span style="color: var(--text-soft); font-weight: 400;">(' . __('site.auth.optional') . ')</span>';
@endphp
@extends('themes.gallery.layout')

@section('content')
    <style>
        .auth-page { max-width: 540px; margin: 0 auto; padding: 4rem 2rem 6rem; }
        .auth-eyebrow {
            font-size: 0.6875rem;
        ...
        .field-hint { display: block; margin-top: .375rem; font-size: 0.75rem; color: var(--text-soft); }

        .shipping-group { border-top: 1px solid var(--hair); padding-top: 1.5rem; margin-top: 1.5rem; }
        ...
        .marketing-label { display: flex; align-items: flex-start; gap: .625rem; cursor: pointer; font-size: 0.875rem; line-height: 1.5; color: var(--text); }
