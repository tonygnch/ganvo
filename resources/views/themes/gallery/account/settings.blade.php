...
@section('content')
    <style>
        ...
        .acct-head h1 { font-family: var(--display); font-weight: 700; font-size: clamp(30px,4vw,46px); letter-spacing: -.02em; margin-top: 12px; }
        .acct-head p { color: var(--muted); font-size: 14px; margin-top: 8px; }
        .flash { background: #d7e6d3; color: #41693f; border-radius: 10px; padding: 12px 16px; margin-bottom: 22px; font-size: 13px; }
        .panel { background: var(--card); border: 1px solid var(--line); border-radius: 16px; padding: 28px; margin-bottom: 18px; }
        .panel > h2 { font-size: 11px; letter-spacing: .08em; text-transform: uppercase; color: var(--muted); margin-bottom: 20px; padding-bottom: 14px; border-bottom: 1px solid var(--line); }
        ...
        .field label { display: block; font-size: 11px; letter-spacing: .06em; text-transform: uppercase; color: var(--muted); margin-bottom: 7px; }
        .field input, .field select { width: 100%; border: 1px solid var(--line); border-radius: 8px; background: var(--bg); padding: 13px 15px; font-family: inherit; font-size: 14px; color: var(--ink); }
        .field input:focus, .field select:focus { outline: none; border-color: var(--accent); }
        .check-row { display: flex; align-items: flex-start; gap: 9px; font-size: 13px; color: var(--muted); cursor: pointer; } .check-row input { accent-color: var(--accent); margin-top: 3px; }
        .save-btn { background: var(--accent); color: #faf7f1; border: 0; border-radius: 8px; padding: 13px 28px; font-weight: 600; font-size: 13px; cursor: pointer; margin-top: 6px; }
        @media (max-width: 540px) { .frow { grid-template-columns: 1fr; } }
    </style>

    <main>
        <div class="acct">
            <div class="acct-head rv"><a href="/account" class="back">← {{ __('site.account.back_to_account') }}</a><h1>{{ __('site.account.settings_title') }}</h1><p>{{ __('site.account.settings_lead') }}</p></div>
            ...
            <form method="post" action="/account/password" class="rv">
                ...
                <div class="panel">
                    ...
                    <div class="frow"><div class="field"><label>{{ __('site.account.new_password') }}</label><input type="password" name="password" required></div><div class="field"><label>{{ __('site.account.confirm_password') }}</label><input type="password" name="password_confirmation" required></div></div>
