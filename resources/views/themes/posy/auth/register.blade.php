...
@section('content')
    <style>
        ...
        .auth .l1 { width: 120px; height: 120px; background: var(--leaf); top: 8%; left: 10%; opacity: .5; animation: floaty 7s ease-in-out infinite; }
        .auth .l2 { width: 90px; height: 90px; background: var(--bloom); bottom: 12%; right: 14%; opacity: .5; animation: floaty 6s ease-in-out infinite .5s; }
        @keyframes floaty { 0%, 100% { transform: translateY(0); } 50% { transform: translateY(-14px); } }
        @media (prefers-reduced-motion: reduce) { .auth .leafdot { animation: none !important; } }

        .auth .card { position: relative; z-index: 2; background: var(--card); border-radius: 14px; width: min(440px, 100%); padding: 46px; text-align: center; box-shadow: 0 40px 80px -40px rgba(40, 50, 31, .5); }
        .auth .card .tape { width: 120px; height: 28px; }
        .auth .card .k { margin-bottom: 8px; }
        .auth .card h1 { font-family: var(--display); font-size: clamp(32px, 4vw, 46px); margin-bottom: 6px; font-weight: 400; }
