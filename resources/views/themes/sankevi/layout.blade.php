...
<head>
    ...
    @if ($theme->on('boot_screen'))
        ...
        <script>
            /*
             ...
             |
             | Here rather than at the foot of the body because a script down
             | there does not run until the stylesheets above it have loaded —
             | on a throttled line that was ten seconds, so a timer registered
             | there could not hold to anything it promised. Measured on Slow
             | 3G: 11s on screen, with the page behind it readable for most of
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* The price has the foot to itself, and room to breathe under it —
           it is the last thing on the plate, so it should not sit on the edge. */
        .pcard .foot { display: flex; align-items: baseline; margin-top: auto; padding-top: 11px; padding-bottom: 3px; border-top: 1px solid var(--line); }
        ...
        /*
         ...
         | column layout above exists to prevent.
         |
         | Dropping "виж" bought back the width it was taking, and at any card
         | wide enough to hold two figures the price now stays at full size. The
         | tier below is for the narrowest phones only, where half of a 320px
         | screen cannot hold "€15.00 – €20.00 / м²" at 19px however the row is
         | arranged.
         |
         | Measured against THE CARD, not the viewport: this same partial runs
         | full-width on a category page and four-up on a desktop, and only the
         | card knows which. nowrap is the guarantee — the figures break as a
         | unit or not at all.
         */
        @container (max-width: 183px) {
