...
class StoreSettings extends Page implements HasForms
{
                ...
                Tabs::make(__('admin.settings.nav.title'))
                    ...
                    ->tabs([
                                ...
                                Section::make(__('admin.settings.section.storefront_effects'))
                                    ...
                                    ->schema([
                                        Select::make('number_animation')
                                            ...
                                            ->options(Store::numberAnimationOptions())
                                            ->default('count')
                                            ->required()
                                            ->native(false)
                                ...
                                Section::make(__('admin.settings.section.announcement'))
                                    ...
                                    ->schema([
                                        ...
                                        Select::make('announcement_speed')
                                            ...
                                            ->options(Store::announcementSpeedOptions())
                                            ->default('normal')
                                            ->native(false)
                        ...
                        Tab::make(__('admin.settings.nav.tab_checkout_shipping'))
                            ...
                            ->schema([
                                /*
                 | HOW AN ORDER ENDS. Store::ORDER_FLOWS has existed since the
                 | enquiry flow was built, but nothing ever exposed it — the
                 | only way to change it was tinker, which is not a setting so
                 | much as a secret. A yard that quotes by hand needs to be
                 | able to say so, and to change its mind.
                 */
                                Section::make(__('admin.settings.section.order_flow'))
                                    ->description(__('admin.settings.section_help.order_flow'))
                                    ->schema([
                                        Radio::make('order_flow')
                                            ->label(__('admin.settings.field.order_flow'))
                                            ->options(fn (): array => [
                                                Store::FLOW_PAYMENT => __('admin.settings.opt.flow_payment'),
                                                Store::FLOW_ENQUIRY => __('admin.settings.opt.flow_enquiry'),
                                            ])
                                            ->descriptions(fn (): array => [
                                                Store::FLOW_PAYMENT => __('admin.settings.help.flow_payment'),
                                                Store::FLOW_ENQUIRY => __('admin.settings.help.flow_enquiry'),
                                            ])
                                            ->default(Store::FLOW_PAYMENT)
                                            ->required(),
                                    ]),
                                ...
                                Section::make(__('admin.settings.section.customer_accounts'))
                                    ...
                                    ->schema([
                                        Radio::make('checkout_mode')
                                            ...
                                            ->options(Store::checkoutModeOptions())
                                            ->default(Store::CHECKOUT_BOTH)
                                            ->required(),
                                        ...
                                        Repeater::make('shipping_methods')
                                            ...
                                            ->schema([
                                                ...
                                                TextInput::make('price_cents')
                                                    ...
                                                    ->required(),
                                                TextInput::make('free_threshold_cents')
    ...
    public function save(): void
    {
        ...
        if (! array_key_exists($annSpeed, Store::ANNOUNCEMENT_SPEEDS)) {
            $annSpeed = 'normal';
        }
        $state['announcement'] = [
        ...
        if (! array_key_exists($anim, Store::NUMBER_ANIMATIONS)) {
            $anim = 'count';
        }
        ...
        $signup = [];
        foreach (Store::SIGNUP_FIELDS as $field) {
